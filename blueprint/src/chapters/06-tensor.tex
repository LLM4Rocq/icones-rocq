...
in \texttt{theories/axioms/saft\_interface.v}. That interface is now
\textbf{discharged}: the Special Adjoint Functor Theorem for $\ICones$
...
\begin{enumerate}
  ...
  \item[(c)] This discharge (\texttt{PLAN.md} \S13.1, milestone
    ``M-SAFT'') brought the tensor / SMC-closed development from the
    staged tier into the axiom-free core; only the Seely-iso existence
    (Chapter~\ref{ch:exponential}) now remains staged.
...
\texttt{Parameter}s of \texttt{theories/axioms/saft\_interface.v}, now
the proved declarations of
