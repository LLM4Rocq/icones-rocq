...
\section{The SAFT construction of \texorpdfstring{$\otimes$}{(x)}, now discharged}%
...
\begin{itemize}
  ...
  \item \emph{The hom-bijection $\Phi$ and its inverse (paper Thm~5.9 /
    ...
    \rocq{Icones.homs.tensor.tensor_curry}\rocqok{}
    $:\ICones(B \otimes C, D) \to \ICones(B, C \lin D)$, the inverse
    \rocq{Icones.homs.tensor.tensor_uncurry}\rocqok{},
    and the two round-trip cancellations
    \rocq{Icones.homs.tensor.tensor_curryK}\rocqok{},
    \rocq{Icones.homs.tensor.tensor_uncurryK}\rocqok{}
    (all proved in \texttt{tensor\_construct.v}).
  \item \emph{Naturality of $\Phi$.} Naturality in $D$ stated
    existentially
    (\rocq{Icones.homs.tensor_construct.tensor_curry_natural_D}\rocqok{}),
    naturality in $B$
    (\rocq{Icones.homs.tensor_construct.tensor_curry_natural_B}\rocqok{}),
    and the \emph{concrete} naturality in $D$ whose witness
    is the explicit hom-functor action $C \lin h$
    (\rocq{Icones.homs.tensor_construct.tensor_curry_natural_post}\rocqok{}).
  ...
  \item \emph{Element-level packaging and the norm equation.} The
    ...
    \rocq{Icones.homs.tensor.tensor_hom_Phi}\rocqok{}
    and the Thm~5.13 norm equation
    \rocq{Icones.homs.tensor.tensor_normM}\rocqok{}
  ...
  \item \emph{Structural-iso existence and their computation laws (paper
    ...
    \rocq{Icones.homs.tensor.tensor_assoc_iso}\rocqok{}
    with
    \rocq{Icones.homs.tensor_iso.tensor_assocE}\rocqok{}, the
    unitors
    \rocq{Icones.homs.tensor.tensor_lunit_iso}\rocqok{}
    /
    \rocq{Icones.homs.tensor.tensor_runit_iso}\rocqok{}
    with
    \rocq{Icones.homs.tensor_iso.tensor_lunitE}\rocqok{} /
    \rocq{Icones.homs.tensor_iso.tensor_runitE}\rocqok{}, and
    the braiding
    \rocq{Icones.homs.tensor.tensor_braid_iso}\rocqok{}
    with
    \rocq{Icones.homs.tensor_iso.tensor_braidE}\rocqok{}.
\end{itemize}
...
\section{Prerequisites for \texorpdfstring{$\otimes$}{(x)} (paper \S5.3)}%
\label{sec:tensor-prereq}

Three pieces of infrastructure feed the tensor development. All three
are genuinely proved.

\subsection{Isomorphisms in \texorpdfstring{$\ICones$}{ICones}}\label{sec:icones-iso}
...
\rocq{Icones.homs.icones_iso.icones_iso}\rocqok\uses{sec:icones}
An isomorphism $B \cong C$ in $\ICones$ is a record
\rocq{Icones.homs.icones_iso.icones_iso} packaging a forward
...
\rocq{Icones.homs.icones_iso.icones_isoP} builds an iso from the two
cancellation equations stated at the level of \texttt{icones\_comp};
the Yoneda-style companion
\rocq{Icones.homs.icones_iso.icones_iso_of_cancel} builds it from
...
\rocq{Icones.homs.icones_iso.icones_iso_refl}\rocqok
\rocq{Icones.homs.icones_iso.icones_iso_sym}\rocqok
\rocq{Icones.homs.icones_iso.icones_iso_trans}\rocqok
\uses{def:icones-iso}
The isos form a groupoid: the identity iso $B \cong B$
(\rocq{Icones.homs.icones_iso.icones_iso_refl}), symmetry $B \cong C
\Rightarrow C \cong B$ by swapping forward and backward
(\rocq{Icones.homs.icones_iso.icones_iso_sym}), and transitivity by
composition (\rocq{Icones.homs.icones_iso.icones_iso_trans}).
Moreover the underlying forward function is bijective: it has the
backward function as a two-sided pointwise inverse
(\rocq{Icones.homs.icones_iso.iso_can},
and its primed variant \texttt{iso\_can'}), hence is injective
(\rocq{Icones.homs.icones_iso.iso_fwd_inj}) and bijective
(\rocq{Icones.homs.icones_iso.iso_fwd_bij}).
\end{lemma}
...
\rocq{Icones.mcones.test_pullback.test_pullback_meas}\rocqok
\rocq{Icones.mcones.test_pullback.test_pullback_meas_bivar}\rocqok
\uses{def:mcone}
...
(\rocq{Icones.mcones.test_pullback.test_pullback_meas}). The
bivariate variant
(\rocq{Icones.mcones.test_pullback.test_pullback_meas_bivar})
...
(\rocq{Icones.mcones.test_pullback.test_pullback_path}). The
...
\rocq{Icones.mcones.test_pullback.test_pullback_cont}\rocqok
...
\rocq{Icones.mcones.test_pullback.cones_hom_image_chain}, and
norm-decreasing,
\rocq{Icones.mcones.test_pullback.cones_hom_image_ub1}); push the
cone-sup through $g$ by $\omega$-continuity of $g$, then apply the
``test-of-sup'' identity
\rocq{Icones.mcones.test_pullback.test_of_sup} (a test is monotone
...
\rocq{Icones.homs.linhom_functor.linhom_map_icones}\rocqok
...
(\rocq{Icones.homs.linhom_functor.linhom_map_iconesE}).
\end{lemma}
...
\begin{itemize}
  \item \emph{$\cones_{\mathrm{hom}}$}
    (\rocq{Icones.homs.linhom_functor.linhom_map_cones}): linearity
    (\rocq{Icones.homs.linhom_functor.linhom_map_linear}),
    norm-decrease $\cnorm{\Lambda f} \le \cnorm{f}$ from the chain
    $\cnorm{g(f(h\,x))} \le \cnorm{f(h\,x)} \le \cnorm{f}\,\cnorm{x}$
    (\rocq{Icones.homs.linhom_functor.linhom_map_norm_le1}), and
    $\omega$-continuity
    (\rocq{Icones.homs.linhom_functor.linhom_map_continuous}) ---
  ...
  \item \emph{$\mcones_{\mathrm{hom}}$}
    (\rocq{Icones.homs.linhom_functor.linhom_map_mcones}): the crux,
    measurable-path preservation \emph{in $f$}
    (\rocq{Icones.homs.linhom_functor.linhom_map_pres_path}). For a
  ...
  \item \emph{$\icones_{\mathrm{hom}}$}
    (\rocq{Icones.homs.linhom_functor.linhom_map_icones}):
    integral-preservation in $f$
    (\rocq{Icones.homs.linhom_functor.linhom_map_pres_int}) ---
...
\rocq{Icones.homs.linhom_functor.linhom_post_icones}\rocqok
\rocq{Icones.homs.linhom_functor.linhom_pre_icones}\rocqok
...
D_1) \to (C \lin D_2)$ is
\rocq{Icones.homs.linhom_functor.linhom_post_icones}, with
$(C \lin g)\,f = g \circ f$
(\rocq{Icones.homs.linhom_functor.linhom_post_iconesE}). The
contravariant pre-composition action $h \lin D :=
\texttt{linhom\_map\_icones}\,h\,(\texttt{icones\_id}\,D) : (C_1 \lin
D) \to (C_2 \lin D)$ is
\rocq{Icones.homs.linhom_functor.linhom_pre_icones}, with
$(h \lin D)\,f = f \circ h$
(\rocq{Icones.homs.linhom_functor.linhom_pre_iconesE}).
\end{definition}

\begin{lemma}[Functoriality of $\lin$, paper Prop 5.8]\label{lem:linhom-functoriality}
\rocq{Icones.homs.linhom_functor.linhom_map_icones_id}\rocqok
\rocq{Icones.homs.linhom_functor.linhom_map_icones_comp}\rocqok
\uses{lem:linhom-map-icones}
The hom-functor preserves identities and composition in $\ICones$:
$(\id_C) \lin (\id_D) = \id_{C \lin D}$
(\rocq{Icones.homs.linhom_functor.linhom_map_icones_id}) and, with
the first slot contravariant,
$(h' \circ h) \lin (g \circ g') = (h' \lin g') \circ (h \lin g)$
(\rocq{Icones.homs.linhom_functor.linhom_map_icones_comp}). Both
...
\subsection{The universal map \texorpdfstring{$\tau$}{tau} and the pure tensor}%
\label{sec:tau}
...
\rocq{Icones.homs.tensor.tensor_curryE}\rocqok
\uses{def:tau}
For $f : \icones_{\mathrm{hom}}\,\Ar\,(B \otimes C)\,D$,
\[
  \Phi(f) \;=\; (C \lin f) \circ \tau_{B,C}
\]
(\rocq{Icones.homs.tensor.tensor_curry_factor}).
and pointwise $\Phi(f)(x)(y) = f(x \otimes y)$
(\rocq{Icones.homs.tensor.tensor_curryE}).
\end{lemma}

\begin{proof}
From the concrete naturality of $\Phi$ in $D$
(\rocq{Icones.homs.tensor_construct.tensor_curry_natural_post},
...
\rocq{Icones.homs.linhom.linhom_map_funE} gives $\Phi(f)(x)(y) =
...
\rocq{Icones.homs.tensor.tensor_mor}\rocqok
...
round-trip
\rocq{Icones.homs.tensor.tensor_curryK}.
\end{proof}
...
\rocq{Icones.homs.tensor.tensor_hom_Phi} (built on the proved
\texttt{tensor\_iso.v} iso \texttt{tensor\_hom\_iso}). Its
forward/backward maps are
\rocq{Icones.homs.tensor.tensor_hom_fwd} /
\rocq{Icones.homs.tensor.tensor_hom_bwd}, and the forward map is
bijective (\rocq{Icones.homs.tensor.tensor_hom_fwd_bij}).
\end{theorem}

At the morphism level, $\Phi = \texttt{tensor\_curry}$ is injective
because \texttt{tensor\_uncurry} is a left inverse: this is
\rocq{Icones.homs.tensor.tensor_curry_inj}\rocqok, proved directly
from \rocq{Icones.homs.tensor.tensor_curryK}. Injectivity
...
\rocq{Icones.homs.tensor.tensor_normME}\rocqok
\uses{def:tau}
...
(\rocq{Icones.homs.tensor.tensor_norm_le}) from norm-decrease of
...
\rocq{Icones.homs.tensor.tensor_normME} re-exports the proved
\rocq{Icones.homs.tensor.tensor_normM} of \texttt{tensor\_hom\_iso.v}
...
\rocq{Icones.homs.tensor.tensor_ext}\rocqok
...
(\rocq{Icones.homs.tensor.tensor_curry_inj}) it suffices that
...
\rocq{Icones.homs.tensor.tensor_ext3}\rocqok
\uses{lem:tensor-ext}
...
\rocq{Icones.homs.smcc.tensor_ext4}\rocqok (one more currying layer
...
\rocq{Icones.homs.smcc.linhom_funZ}).
\end{lemma}
...
\rocq{Icones.homs.smcc.tensor_assoc}\rocqok
\rocq{Icones.homs.smcc.tensor_lunit}\rocqok
\rocq{Icones.homs.smcc.tensor_runit}\rocqok
\rocq{Icones.homs.smcc.tensor_braid}\rocqok
\uses{def:icones-iso, sec:tensor-staging}
The associator $\alpha_{A,B,C} : (A \otimes B) \otimes C \cong A
\otimes (B \otimes C)$
(\rocq{Icones.homs.smcc.tensor_assoc}), the left/right unitors
$\lambda_A : 1 \otimes A \cong A$
(\rocq{Icones.homs.smcc.tensor_lunit}) and $\rho_A : A \otimes 1 \cong
A$ (\rocq{Icones.homs.smcc.tensor_runit}), and the braiding
$\sigma_{A,B} : A \otimes B \cong B \otimes A$
(\rocq{Icones.homs.smcc.tensor_braid}) are the
...
\]
namely \rocq{Icones.homs.smcc.tensor_assocEp},
\rocq{Icones.homs.smcc.tensor_lunitEp},
\rocq{Icones.homs.smcc.tensor_runitEp},
\rocq{Icones.homs.smcc.tensor_braidEp} (here $u \in 1 = \Rnn$ acts by
scaling). These restate the proved computation laws
\rocq{Icones.homs.tensor_iso.tensor_assocE} etc.\ of \texttt{tensor\_iso.v}
(\S\ref{sec:tensor-staging}).
\end{definition}

The bifunctor action computes on pure tensors:
$(f \otimes g)(x \otimes y) = (f\,x) \otimes (g\,y)$, namely
\rocq{Icones.homs.smcc.tensor_morE}\rocqok, derived from the
round-trip \rocq{Icones.homs.tensor.tensor_uncurryK} and
Eq~5.1.
...
\rocq{Icones.homs.smcc.tensor_braid_invol}\rocqok
...
\rocq{Icones.homs.smcc.tensor_triangle}\rocqok
...
\rocq{Icones.homs.smcc.tensor_pentagon}\rocqok
...
\rocq{Icones.homs.smcc.tensor_hexagon}\rocqok
\uses{def:struct-isos, lem:tensor-ext3}
...
\subsection{Theorem 5.15: \texorpdfstring{$\ICones$}{ICones} is symmetric monoidal closed}%
\label{sec:thm515}

\begin{theorem}[Paper Theorem 5.15]\label{thm:thm515}
\rocq{Icones.homs.smcc.ICones_smcc}\rocqok
...
\rocq{Icones.homs.smcc.ICones_SMCC}, whose canonical witness
\rocq{Icones.homs.smcc.ICones_smcc} populates every field with the
proved data:
\begin{itemize}
...
\rocq{Icones.homs.smcc.ICones_smcc} just assembles them. The structural
...
(\rocq{Icones.homs.icones_iso.icones_iso}), the Def~3.13 test-pullback
characterisation
(\rocq{Icones.mcones.test_pullback.test_pullback_meas},
\rocq{Icones.mcones.test_pullback.test_pullback_meas_bivar},
\rocq{Icones.mcones.test_pullback.test_pullback_cont}), and the
morphism-level internal-hom functor of Prop~5.8
(\rocq{Icones.homs.linhom_functor.linhom_map_icones}, its one-sided
...
\rocq{Icones.homs.smcc.ICones_smcc} witnessing Theorem~5.15) are all
\Qed{}'d.
...
its naturality (\rocq{Icones.homs.tensor.tensor_curry},
\rocq{Icones.homs.tensor.tensor_uncurry},
\rocq{Icones.homs.tensor_construct.tensor_curry_natural_post}), the Thm~5.12
iso (\rocq{Icones.homs.tensor.tensor_hom_Phi}), the Thm~5.13 norm
equation (\rocq{Icones.homs.tensor.tensor_normM}), and the structural
isos of Eqs~5.2--5.4
(\rocq{Icones.homs.tensor.tensor_assoc_iso},
\rocq{Icones.homs.tensor.tensor_lunit_iso},
\rocq{Icones.homs.tensor.tensor_runit_iso},
\rocq{Icones.homs.tensor.tensor_braid_iso}) with their computation
