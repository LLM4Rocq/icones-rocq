...
centrepiece of this chapter. (The CBV-side mass-one identity is
currently pending in \autoref{ch:cbv}: \texttt{ne\_fix} in
\texttt{ppl\_cbv.v} is a \texttt{precone\_zero} placeholder.)
...
of this section. (The CBV-side parallel, formerly stated through the
\texttt{em\_fix\_arr.v} machinery, is currently pending the
\texttt{ne\_fix} port in \texttt{ppl\_cbv.v}.) The surface term is
unchanged:
\[
  \mathtt{ex\_geom}
  \;:=\;
...
CBV-side mass-one identity is currently pending in \autoref{ch:cbv}
(see the \texttt{precone\_zero} placeholder for \texttt{ne\_fix} in
\texttt{ppl\_cbv.v}); once that lands, the two interpretations'
identities will form a triple with the bridge. All formalisation
