...
%     CBV ex_geom_arr_mass_one of Chapter 9, closed via the unary
...
denotation of the geometric program has total mass $1$, the CBN-side
parallel of the CBV mass-one identity of \autoref{thm:cbv-ex-geom-arr-mass-one}
which we discuss in this chapter as the second centrepiece.
...
\rocq{Icones.stable.fixpoint.Yfix_fix}: this is the CBN-side analogue
of \autoref{thm:cbv-yfix-fun}, available \emph{unconditionally} at any
function type $\sem{\mathtt{tfun}\,A\,B}_n = \sem{A}_n \Rightarrow_s
\sem{B}_n$, no bilinear-bridge obligation arising.
\end{lemma}
...
The headline CBV mass-one identity of
\autoref{thm:cbv-ex-geom-arr-mass-one}, however, sidesteps the bridge
through a manual Kleene chase that is cheaper than the bridge at this
single example.
\end{remark}
...
The CBV mass-one identity for $\mathtt{ex\_geom}$ of
\autoref{lem:ppl-ex-geom} was an existence statement
(``the total mass is $1$'') for the CBV denotation. The CBN-side
parallel is the focus of this section. The surface term is unchanged:
\[
  \mathtt{ex\_geom}
  \;:=\;
...
\]
the geometric counter of \autoref{lem:ppl-ex-geom}.
...
$\phi_{\mathrm{CBN}}^{\mathrm{geom}}$ from $\bot$, exactly as for the
CBV \texttt{ex\_geom\_arr} in \autoref{lem:ppl-ex-geom}. The
per-iterate mass is closed in three steps.
...
\autoref{thm:cbn-ex-geom} (the CBN headline) --- with
\autoref{thm:cbv-ex-geom-arr-mass-one} of \autoref{ch:cbv} (the CBV
headline) form a coherent triple. The CBV headline establishes that
the geometric program has mass $1$ under the EM(!) / direct-style
reading; the CBN headline establishes the same identity under the
$\mathbf{SCones}$ / co-Kleisli reading; and the bridge establishes the
structural unblocker that, when packaged, would deliver both
\emph{generically} (not example-by-example), for arbitrary bilinear
arithmetic and recursive bodies. The headlines are example-specific
shortcuts that the bridge would generalise; the bridge is the math
content that the headlines avoid by exploiting the geometric program's
specific shape (Bernoulli + Dirac constants). All three live in the
axiom-free core.
