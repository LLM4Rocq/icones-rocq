...
%
% Wave 2 is populated. The two MVP money-theorem statements
...
proof of the MVP money theorem reduces, via paper Theorem~6.1 (the
...
the full $\mcones$-iso packaging; we defer it. The MVP application
...
\rocq{Icones.kernels.thm65.Skern_to_ICones_mor_norm_le1}).
The unfolding lemma
\rocq{Icones.kernels.thm65.Skern_to_ICones_mor_E} confirms
...
\begin{itemize}
  \item \rocq{Icones.kernels.thm65.Skern_to_ICones_mor_id}:
    $\mathsf{Klin}(\delta_X) = \id_{\FMeas(X)}$ in $\ICones$;
  \item \rocq{Icones.kernels.thm65.Skern_to_ICones_mor_comp}:
...
\rocq{Icones.kernels.thm65.Skern_to_ICones_faithful}%
...
\rocq{Icones.kernels.thm65.Skern_to_ICones_full}%
...
\rocq{Icones.kernels.thm65.icones_to_skern_norm_le1}). So $\kappa$
is a substochastic kernel.

Now $f$ itself, viewed as a $\linhom$ via the helper
\rocq{Icones.kernels.thm65.icones_to_linhom} (which repackages the
...
\begin{theorem}[Paper Theorem~6.5, MVP money theorem]%
...
Theorem~\ref{thm:thm65}, and on the axiom budget of the entire MVP.
...
norm-decrease constraint. We sidestep the difficulty for the MVP by
stating Theorem~\ref{thm:thm65} directly for $\Skern_{\mathrm{hom}}$,
which already carries the unit-norm constraint. The full
$\mcones$-iso packaging and naturality in $X$ and $B$ are deferred
beyond the MVP.

\paragraph{Axiom budget.}
The MVP money theorem,
\rocq{Icones.kernels.thm65.Skern_to_ICones_fully_faithful}, is
...
\paragraph{Closing the PLAN.md MVP target.}
Theorem~\ref{thm:thm65} closes the MVP target spelled out in
\texttt{PLAN.md}: paper \S2 (M1, $\Cones$), \S3 (M2, $\MCones$,
$\FMeas$, $\Path$), \S4 (M3, $\ICones$, Pettis, cone-Fubini), \S5.1
+ \S5.2 (M4, the internal hom $\lin$), and \S6 + MVP (M5, $\Skern$,
Theorem~6.5) are all formalised. The development comprises
$28$ commits up to the MVP-closing commit, plus the post-MVP audit
and the present blueprint commits. The deferred items
(\S5.3--\S5.5, \S7, \S8, \S9, \S10) are explicitly outside the MVP
scope and remain subjects for follow-up work.
