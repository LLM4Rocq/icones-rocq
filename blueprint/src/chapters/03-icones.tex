...
$\int_X \beta(r)\mu(dr)$. In Rocq, once the existence witness
\texttt{hex} is in hand, the value is extracted via classical
indefinite description as \rocq{Icones.icones.pettis.path_integral}
applied to \texttt{hex}.
...
\subsection{Bilinearity in \texorpdfstring{$\beta$}{beta} and \texorpdfstring{$\mu$}{mu}}\label{ssec:bilinear}
...
\subsection{\texorpdfstring{$\omega$}{omega}-continuity in \texorpdfstring{$\beta$}{beta} and \texorpdfstring{$\mu$}{mu}}\label{ssec:omega-cont}
...
\section{\texorpdfstring{$\FMeas(X)$}{FMeas(X)} is integrable --- paper Thm 4.5}\label{sec:fmeas-icone}
...
\section{\texorpdfstring{$\Path(X, B)$}{Path(X, B)} is integrable --- paper Thm 4.12}%
...
helpers
\rocq{Icones.icones.fubini.beta_uncast_bound},
\rocq{Icones.icones.fubini.beta_uncast_secY},
\rocq{Icones.icones.fubini.beta_uncast_secX},
\rocq{Icones.icones.fubini.beta_uncast_jointX} and
\rocq{Icones.icones.fubini.beta_uncast_jointY} discharge each side
condition.
\end{proof}

\section{\texorpdfstring{$\ICones$}{ICones} is complete and well-powered}\label{sec:icones-cat}
