...
% cpD_sample_var_dirac are all axiom-clean.  Higher-order CBV is also
% interpreted, in fine-grain Moggi-CBV style, by theories/homs/ppl.v:
% function types denote the EM(!) Kleisli exponential
%   ⟦tfun A B⟧ = !̃(U A ⊸ U B)
% — the representing object for Hom_EM(- × A, T B) with T = !̃ ∘ U.
% The four-step natural bijection (cofree U ⊣ !̃, U strict monoidal via
% cbv_U_prod, SMCC closure of ICones, cofree U ⊣ !̃ again) yields lambda
% and application directly; application is a computation INTRINSICALLY
% in Moggi-CBV (that is what applying an effectful function is), NOT a
% workaround for a missing structure.  ppl.v includes the QBS-style
% showcase ex_random_constant — a distribution over a function space
% P(R → R), interpreted axiom-free.  EM(!) is NOT cartesian closed and
% is not expected to be (a structural fact); Kleisli exponentials are
% what Moggi-CBV needs, and that is what ppl.v uses.  Adequacy /
...
This is \texttt{theories/homs/ppl.v}, fine-grain Moggi-CBV, axiom-free.
...
companion lives in \texttt{theories/homs/ppl.v} and ports the
...
\rocq{Icones.homs.ppl.ex_random_constant_denot_E}\rocqok
\rocq{Icones.homs.ppl.ex_random_constant_value_E}\rocqok
\uses{thm:cbv-model}
