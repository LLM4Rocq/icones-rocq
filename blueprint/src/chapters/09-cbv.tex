...
(\autoref{sec:cbv-ppl}). The QBS showcase
\rocq{Icones.homs.ppl.ex_random_constant} --- a distribution over a
function space, \texttt{P}$(\mathbb{R}\to\mathbb{R})$ --- interprets
...
\paragraph{The calculus.}
The syntax is the previous one extended with arrows. Types are
\rocq{Icones.homs.ppl.ty'} ($\mathtt{tunit'}$, $\mathtt{tbase'}\,X$,
$\mathtt{tprod'}$, and $\mathtt{tfun}\,A\,B$), values
\rocq{Icones.homs.ppl.vl'} are the previous ones plus
$\mathtt{v\_lam}$, and computations \rocq{Icones.homs.ppl.cp'} are
the previous ones plus a Moggi / CBPV-style \emph{value-to-computation}
application $\mathtt{c\_app}\,V\,W : T\,B$ (the result lives in
$T\,B$, not $B$).
...
\paragraph{Soundness.}
The LNL $\beta$-rule
\rocq{Icones.homs.ppl.adj_phi_cpD'_app_lam} records
$\sem{\mathtt{c\_app}\,(\mathtt{v\_lam}\,V)\,W}$ at the underlying
$\ICones$-level. Companion lemmas
\rocq{Icones.homs.ppl.vlD'_lamE}\,/\,
\rocq{Icones.homs.ppl.adj_phi_lam_coalg} and
\rocq{Icones.homs.ppl.cpD'_appE}\,/\,
\rocq{Icones.homs.ppl.adj_phi_app_kleisli} factor the value side
through $\Phi$\,/\,$\Psi$.
...
\[
  ...
  \mathtt{P}\,(X \to X),
\]
a distribution over a function space
(\rocq{Icones.homs.ppl.ex_random_constant}), is interpreted axiom-free
in the LNL setting --- its denotation
\rocq{Icones.homs.ppl.ex_random_constant_denot} reduces to the
constant-lambda value
(\rocq{Icones.homs.ppl.ex_random_constant_denot_E}). Classical
measure-theoretic semantics famously cannot accommodate such an object;
the LNL bypass on top of the integrable-cones model can.
...
(\autoref{sec:cbv-ppl}, \rocq{Icones.homs.ppl.ex_random_constant_denot_E}).
