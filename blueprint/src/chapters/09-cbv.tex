...
% interpreted via the LNL / CBPV bypass of theories/homs/ppl.v: function
% types live via the Girard decomposition A → B = !A ⊸ B (the linear hom
% out of !A, a cofree coalgebra), so no value-CCC is needed.  ppl.v
% includes the QBS-style showcase ex_random_constant — a distribution
% over a function space P(R → R), interpreted axiom-free.  ONE honest
% remainder: value-level application v_app (vs computation-level c_app)
% would need the SMCC evaluation map !A ⊸ B ⊗ !A → B to be a coalgebra
% morphism (Melliès §6.3 / Bénabou); documented in ppl.v as APP_NOTE.
% Adequacy / normalization / full-abstraction also out of scope here.
...
\paragraph{Higher-order CBV via Girard / CBPV.}
The value category $\mathrm{EM}(\mathord{!})$ is cartesian but
\emph{not yet known to be closed}. Rather than proving a value-CCC, we
follow the LNL / CBPV bypass that ports the QBS-style PPL of
LLM4Rocq/mathcomp-qbs: function types live via the Girard decomposition
$A \to B \;=\; \mathord{!}A \multimap B$, a cofree coalgebra of the
\emph{linear} hom out of $\mathord{!}A$; lambda is
$\mathtt{tensor\_curry}$ of the body with $\mathrm{der}$ inserted, then
$\Psi$ (the adjunction transpose) to land in $\mathrm{EM}(\mathord{!})$;
application is a CBPV-style \emph{computation} $T B$ via
$\Phi^{-1}$. This is \texttt{theories/homs/ppl.v}, also axiom-free
(\autoref{sec:cbv-ppl}). The QBS-paper showcase
\texttt{ex\_random\_constant} --- a distribution over a
function space, $\mathtt{P}(\mathbb{R}\to\mathbb{R})$ --- interprets
soundly in the LNL setting.

\paragraph{The one honest remainder.}
\emph{Value-level} application $v_{\mathrm{app}}$ (as a value, not a
computation) would require the SMCC evaluation map
$\mathrm{ev} : (\mathord{!}A \multimap B) \otimes \mathord{!}A \to B$
to itself be a coalgebra morphism (Melli\`es \S6.3 / B\'enabou). That
is documented in \texttt{ppl.v} as the
\texttt{APP\_NOTE}. Adequacy, normalization, and full-abstraction
...
demonstration. The higher-order companion via the LNL / CBPV bypass
follows in \autoref{sec:cbv-ppl}. The deliverable here is a sound
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{A higher-order PPL via the Girard / CBPV bypass}%
\label{sec:cbv-ppl}

\autoref{sec:cbv-calculus} stops at first order because a closed
structure on the value category $\mathrm{EM}(\mathord{!})$ has not
been proved (\autoref{sec:cbv-status}). A higher-order companion is
nevertheless available, by the standard \emph{LNL / CBPV} bypass: it
ports the \texttt{ppl} branch of \texttt{LLM4Rocq/mathcomp-qbs} (the
QBS-based PPL) into the cones model, interpreting function types via
the Girard decomposition rather than a value-CCC. All of this lives in
\texttt{theories/homs/ppl.v} and is axiom-free.

\paragraph{The calculus.}
The syntax is the previous one extended with arrows. Types
...
computations \texttt{cp}$'$ are the previous ones plus a Moggi /
CBPV-style \emph{value-to-computation} application
$\mathtt{c\_app}\,V\,W : T\,B$ (the result lives in $T\,B$, not $B$).

\paragraph{The interpretation.}
Function types denote a \emph{cofree coalgebra of a linear hom}:
\[
  \sem{\mathtt{tfun}\,A\,B}
  \;\;=\;\;
  \widetilde{\mathord{!}}\,(\mathord{!}\,\sem{A} \multimap \sem{B}),
\]
where $\multimap$ is the SMC closure of $\ICones$
(\autoref{thm:thm512}). $\mathtt{v\_lam}$ denotes $\Psi$ applied to
$\mathtt{tensor\_curry}$ of the body precomposed with $\mathrm{der}$,
landing in $\mathrm{EM}(\mathord{!})$ as the
$\mathtt{tfun}$-cofree coalgebra; $\mathtt{c\_app}$ denotes the
Kleisli-style $\Phi^{-1}$ on the linear evaluation map.

\paragraph{Soundness.}
The LNL $\beta$-rule \texttt{adj\_phi\_cpD}$'$\texttt{\_app\_lam}
records $\sem{\mathtt{c\_app}\,(\mathtt{v\_lam}\,V)\,W}$ at the
underlying $\ICones$-level. Companion lemmas
\texttt{vlD}$'$\texttt{\_lamE} / \texttt{adj\_phi\_lam\_coalg} and
\texttt{cpD}$'$\texttt{\_appE} / \texttt{adj\_phi\_app\_kleisli}
factor the value side through $\Phi$ / $\Psi$. The PPL-monad
identity $\sem{\mathtt{c\_sample}\,V} = \sem{\mathtt{c\_ret}\,V}$ on
base types (\texttt{cpD}$'$\texttt{\_sample\_ret}) survives the
...
\[
  ...
  \mathtt{P}\,(X \to X),
\]
a distribution over a function space, is interpreted axiom-free in the
LNL setting --- its denotation reduces to the constant-lambda value.
Classical measure-theoretic semantics famously cannot accommodate such
an object; the LNL bypass on top of the integrable-cones model can.
...
via the PPL-monad identities
$\mathtt{c\_sample} = \mathtt{c\_ret}$ on base types and the
$\mathtt{let}$-of-$\mathtt{return}$ left-unit law.
\end{lemma}

\begin{proof}
The companion facts \texttt{adj\_phi\_lam\_coalg} and
\texttt{adj\_phi\_app\_kleisli} factor lambda and application through
the adjunction transpose $\Phi$; the on-base-type identity
$\mathtt{c\_sample} = \mathtt{c\_ret}$ is a direct consequence of the
$\widetilde{!}\circ U$-induced monad's left unit, since the base
coalgebra is the cofree $\widetilde{!}(\FMeas\,X)$ and $\mathtt{sample}$
is interpreted as $\eta = \mathrm{adj\_unit}$ there.
\end{proof}

\paragraph{What is not yet covered.}
\emph{Value-level} application $v_{\mathrm{app}}$ (returning $B$, not
$T B$) would require the SMCC evaluation map
$\mathrm{ev}_{!A,B} : (\mathord{!}A \multimap B) \otimes \mathord{!}A
\to B$ to be a \emph{coalgebra morphism}, in the sense of Melli\`es
\S6.3 / B\'enabou (the monoidal cofree adjunction's internal-hom
counit). This is recorded as \texttt{APP\_NOTE} in
\texttt{ppl.v}. Also not interpreted (orthogonal): booleans /
coproducts (the cones model has no native sum type), and the
...
(\autoref{sec:cbv-calculus}, \autoref{thm:cbv-soundness}); and a
higher-order companion via the LNL / CBPV bypass, including the QBS
flagship showcase $\mathtt{ex\_random\_constant}$
(\autoref{sec:cbv-ppl}, \autoref{lem:ppl-random-constant}).

\paragraph{Open --- the one honest remainder.}
A genuine value-CCC on $\mathrm{EM}(\mathord{!})$ is \emph{not yet
proved}. The higher-order calculus of \autoref{sec:cbv-ppl} sidesteps
this via the LNL / CBPV bypass (function types via Girard
$A \to B = \mathord{!}A \multimap B$), but \emph{value-level}
application would require the SMCC evaluation map to lift to a
coalgebra morphism (the \texttt{APP\_NOTE} of \texttt{ppl.v}, Melli\`es
\S6.3 / B\'enabou). Adequacy, normalization, and full-abstraction are
