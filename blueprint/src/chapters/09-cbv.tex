...
the second projection (\rocq{Icones.homs.em_cartesian.em_proj2}) and
...
\rocq{Icones.homs.tensor.tensor_uncurry} of the outer-linhom
...
higher-order content shown here). The QBS-paper's
\texttt{qbs\_normalize} / Bayesian-posterior showcase --- a different
observable than the ``random function'' examples reproduced here,
not interpreted by either of \autoref{lem:ppl-random-constant} or
...
paper-\S6 follow-up
\rocq{Icones.homs.bilin.int_to_linhom_pres_path_in_cone}
(\autoref{thm:cbv-fmeas-lax}); and the higher-order direct-style
multi-variable QBS-mirror PPL, with the two headline reduction lemmas
\rocq{Icones.programs.ppl.ex_random_constant_denot_E} (the QBS
flagship, \autoref{lem:ppl-random-constant}) and
