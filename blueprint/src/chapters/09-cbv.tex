...
%   theories/programs/infra/em_fix.v   the zero-seeded linear Kleene
%                                      operator on the linhom cone:
%                                      Phi_fun / linhom_lfp / Yfix_fun_lin
%                                      and their ball / monot. /
%                                      ω-continuity laws.  Kept as the
%                                      DOCUMENTED CONTRAST: the operator
%                                      is provably degenerate
%                                      (em_fix_value.v::Yfix_fun_lin_eq0,
%                                      the zero linhom on every input);
%                                      no longer used by the interpreter.
%   theories/programs/infra/em_fix_value.v   the repaired seeded CBV
%                                      value-fixpoint: the degeneracy
%                                      record Yfix_fun_lin_eq0; the
...
% via the Seely transport of em_fix_mr.v; the old zero-seeded
% Yfix_fun_lin of em_fix.v is provably degenerate and survives only as
% the documented contrast) and
...
records three things. First, the \emph{degeneracy} of the previous
operator $\mathtt{Yfix\_fun\_lin}$ of
\texttt{theories/programs/infra/em\_fix.v}: its Kleene step is linear
in the previous approximant and its chain is seeded at the cone-zero of
the linhom cone, so every iterate --- and the supremum --- is the zero
linhom (\autoref{thm:cbv-yfix-degenerate}). The bottom of a CBV
...
\subsection{The zero-seeded operator $\mathtt{Yfix\_fun\_lin}$}%
\label{ssec:cbv-phifun}

We first record the operator that is being replaced, since it is the
degeneracy contrast (it is no longer used by the interpreter
anywhere: its last consumer, the
...
\subsection{The Kleene supremum $\mathtt{Yfix\_fun\_lin}$, and the
degeneracy theorem}%
...
\begin{definition}[The zero-seeded operator $\mathtt{Yfix\_fun\_lin}$]%
...
$\delta : \Gamma \to \Gamma \otimes \Gamma$ and a body
$M : \Gamma \otimes B \to B$, the zero-seeded operator is
\[
...
The fixpoint equation is correct but vacuous, as the next theorem
shows: the operator never produces anything but zero.

\begin{theorem}[Degeneracy: $\mathtt{Yfix\_fun\_lin} = 0$, always]%
\label{thm:cbv-yfix-degenerate}
\rocq{Icones.programs.infra.em_fix_value.Yfix_fun_lin_eq0}\rocqok
...
the supremum of the constantly-zero chain is zero.
\end{proof}
...
(formerly the honest-scope placeholder was the degenerate
$\mathtt{Yfix\_fun\_lin}$ of \autoref{thm:cbv-yfix-degenerate}).
\end{definition}
...
\begin{itemize}
  ...
  \item the $\mathtt{ne\_fix\_mr}$-at-$\mathtt{tprod}$ computation law
    ...
    retired $\mathtt{Yfix\_fun\_lin}$ placeholder.
\end{itemize}
...
\rocq{Icones.programs.infra.em_fix_value.Yfix_fun_lin_eq0}
...
degenerate $\mathtt{Yfix\_fun\_lin}$); $\mathtt{tbool}$ is the
\S9.7-style
