...
This section assembles the full semantic analysis of the
rejection-sampling program: a \emph{higher-order} (the recursive
function abstracts over the acceptance continuation),
\emph{probabilistic} (continuous $\mathtt{sample}$ plus
value-dependent $\mathtt{Bernoulli\_f}$), \emph{non-terminating}
(rejection recurses, and may loop forever) program whose CBV
denotation is identified in closed form: the normalised posterior of
the prior given the soft acceptance predicate. Everything is
