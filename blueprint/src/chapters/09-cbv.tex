...
morphisms (the \S9.2 $\mathrm{Yfix}$ of \autoref{lem:cbn-recursion},
...
construction. The CBN and the CBV recursions thereby share the same
stable-fixpoint core: the chain supremum of $\mathtt{fix\_comb}$ is
computed through the very $\mathrm{Yfix}$ that powers the CBN
$\mathtt{ne\_fix}$ clause.
...
\uses{lem:cbv-fix-chain-laws, thm:scones-ccc, lem:cbn-recursion}
In $\mathbf{SCones}$:
\begin{itemize}
  ...
  \item $\mathtt{fix\_value} = \mathrm{Yfix}_A \circ \mathtt{fix\_body}
    ...
    \rocq{Icones.stable.fixpoint.Yfix} of \autoref{lem:cbn-recursion}.
\end{itemize}
...
The CBN and the CBV recursions now share the same stable-fixpoint
core: the CBN $\mathtt{ne\_fix}$ clause applies the \S9.2
\rocq{Icones.stable.fixpoint.Yfix} directly at the SCones function
space (\autoref{lem:cbn-recursion}), while the CBV
$\mathtt{fix\_comb}$ \emph{factors through the same operator} --- its
chain supremum is computed by $\mathrm{Yfix}_A$ applied to the curried
stable body (\autoref{def:cbv-fix-value}), then transported into
$\mathrm{EM}(\mathord{!})$ by $\mathrm{lin}$ and $\mathtt{adj\_psi}$.
The two readings differ only in the packaging: SCones-morphisms on
stable cones feeding \rocq{Icones.programs.ppl_cbn.eD_CBN_fix_E} for
CBN; an EM-morphism between cofree coalgebras feeding
...
constant zero. The closed-form mass identities --- including the CBV
twin of the CBN
\rocq{Icones.programs.ppl_cbn_geom.ex_geom_CBN_mass_one}
(\autoref{thm:cbn-ex-geom}) --- are now proved in
\autoref{sec:cbv-geom-mass}.
...
closing the CBV side of the mass-identity programme that
\autoref{ch:cbn} opened with the CBN
\rocq{Icones.programs.ppl_cbn_geom.ex_geom_CBN_mass_one}.
...
(\rocq{Icones.programs.infra.affine_cascade.affine_iter_cvg_half}).
Together with the CBN twin
\rocq{Icones.programs.ppl_cbn_geom.ex_geom_CBN_mass_one}
(\autoref{thm:cbn-ex-geom}), \emph{both} interpretations of the shared
surface program now prove mass one for the geometric counter.
\end{theorem}
...
\rocq{Icones.programs.ppl_cbv.var_lookup_cbv}; the shared surface
...
\rocq{Icones.programs.infra.bool_cone_coalg.bool_cone_coalg}; and the
CBV-side mass identities --- including the geometric twin of the
CBN-side \rocq{Icones.programs.ppl_cbn_geom.ex_geom_CBN_mass_one}
(\autoref{thm:cbn-ex-geom}) --- are proved in
\autoref{sec:cbv-geom-mass}. What remains as follow-up:
\begin{itemize}
  \item $\mathtt{ne\_fix\_mr}$ at $\mathtt{tprod}$ body types still
    ...
    (\autoref{def:cbv-fix-clause});
  \item closed-form CBV marginal identities for the QBS trio
    \rocq{Icones.programs.examples.ex_random_constant} /
    \rocq{Icones.programs.examples.ex_random_linear} /
    \rocq{Icones.programs.examples.ex_bayes_linear}, mirroring the
    CBN-side headlines --- now unblocked by the let-at-sample
    integral law (\autoref{thm:cbv-let-sample}), which supplies
    exactly the integral reading of the $\mathtt{let}$-$\mathtt{sample}$
    prefix these programs share.
\end{itemize}
