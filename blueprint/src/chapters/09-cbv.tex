...
%                                      ne_fix (and ne_fix_mr at tfun body
%                                      types) routes through fix_comb of
%                                      em_fix_value.v — the seeded CBV
%                                      value-fixpoint combinator, an
%                                      EM-morphism by construction:
...
%                                      honest-scope ne_fix_mr clause at
%                                      tprod body types.
...
%   theories/programs/infra/cbv_qbs_marginals.v   the CBV marginal
%                                      headlines of the QBS trio:
...
%   theories/programs/examples.v       twelve surface PPL programs (no
%                                      denotation reduction lemmas; this
%                                      file is the surface program pool
...
% at tfun body types) routed through fix_comb of em_fix_value.v (the
% seeded CBV value-fixpoint combinator; the old zero-seeded
% Yfix_fun_lin of em_fix.v is provably degenerate and survives only as
% the documented contrast / the honest ne_fix_mr-at-tprod clause) and
...
\autoref{sec:cbv-laws} --- paired exactly with the unnormalised Bayes
posterior of the score program by the QBS-marginal identities of
\autoref{ssec:cbv-qbs-marginals}.
...
\rocq{Icones.programs.examples.ex_bayes_linear}, the recursive
\rocq{Icones.programs.examples.ex_loop} / \texttt{ex\_geom} /
\texttt{ex\_almost\_loop}, and the rejection-sampling headline
...
\texttt{theories/programs/infra/em\_fix\_value.v} and
\texttt{theories/programs/infra/cbv\_fix\_unfold.v} (the seeded CBV
value-fixpoint combinator and its unfolding laws,
...
$\mathtt{ne\_fix\_mr}$ at function body types) to the value-fixpoint
\emph{combinator}
\[
  \mathtt{fix\_comb}
  \;:\;
...
records two things. First, the \emph{degeneracy} of the previous
...
computed through the $\mathrm{Yfix}$ of \autoref{lem:yfix}.
...
degeneracy contrast and still serves the honest-scope
$\mathtt{ne\_fix\_mr}$-at-$\mathtt{tprod}$ clause
(\autoref{ssec:cbv-fix-wiring}). The step operator works at the linhom
...
\uses{thm:cbv-fix-comb, thm:cbv-yfix-degenerate}
...
$\mathtt{tprod}$-of-frees it still routes through the degenerate
$\mathtt{Yfix\_fun\_lin}$
(\rocq{Icones.programs.ppl_cbv.eD_fix_mr_prod_E}) --- an honest scope
limitation: repairing the product case needs the Seely transport of
$\mathtt{fix\_comb}$ along
$\mathrm{EM\_prod}(\widetilde{\mathord{!}}X, \widetilde{\mathord{!}}Y)
\cong \widetilde{\mathord{!}}(X \otimes Y)$, which is open.
\end{definition}
...
\uses{def:cbv-fix-clause, thm:cbv-fix-value}
...
\begin{itemize}
  ...
  \item $\mathtt{ne\_fix\_mr}\,s\,t\,\_\,M$ (free-coalg-type
    ...
    $\mathtt{ne\_fix}$
    (\rocq{Icones.programs.ppl_cbv.eD_fix_mr_fun_E}); at
    $\mathtt{tprod}$-of-frees still the degenerate
    $\mathtt{Yfix\_fun\_lin}$
    (\rocq{Icones.programs.ppl_cbv.eD_fix_mr_prod_E}) pending the
    Seely transport of $\mathtt{fix\_comb}$
    (\autoref{def:cbv-fix-clause}).
\end{itemize}
...
The twelve surface PPL programs in
...
are the subject of \autoref{sec:cbv-geom-mass}. The QBS-style
examples ---
...
(\autoref{ssec:cbv-qbs-marginals}): the closed-form CBV marginal
identities of the three non-recursive QBS-mirror examples, including
the unnormalised Bayes posterior and its exact pairing with the
rejection sampler.
...
\subsection{The CBV QBS marginals}\label{ssec:cbv-qbs-marginals}

The three non-recursive QBS-mirror examples
...
CBV marginal identities, proved in
\texttt{theories/programs/infra/cbv\_qbs\_marginals.v} on top of the
...
\label{lem:cbv-qbs-kit}
\rocq{Icones.programs.infra.cbv_qbs_marginals.coalg_e_FMeas_prob}\rocqok
\rocq{Icones.programs.infra.cbv_qbs_marginals.em_proj1_mor_unitE}\rocqok
\rocq{Icones.programs.infra.cbv_qbs_marginals.em_proj1_mor_probE}\rocqok
...
(\rocq{Icones.programs.infra.cbv_qbs_marginals.coalg_e_FMeas_prob}:
...
component silently
(\rocq{Icones.programs.infra.cbv_qbs_marginals.em_proj1_mor_probE}),
and discards a non-setlike $\mathtt{tunit}$-typed component as its
scalar weight
(\rocq{Icones.programs.infra.cbv_qbs_marginals.em_proj1_mor_unitE})
--- the mechanism by which a score result becomes a scaling factor.
\end{lemma}

\begin{theorem}[The unnormalised Bayes posterior]%
\label{thm:cbv-qbs-bayes}
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_bayes_linear_cbv_posterior}\rocqok
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_bayes_linear_cbv_mass}\rocqok
\uses{lem:cbv-let-sample-mu, lem:cbv-qbs-kit}
The closed CBV denotation of $\mathtt{ex\_bayes\_linear}$ (sample
$m \sim \mu$, score by a measurable density $f : \mathbb{R} \to
[0,1]$, return $m$) is the \emph{unnormalised posterior}: for every
measurable $U$,
\[
  \sem{\mathtt{ex\_bayes\_linear}}(1)(U) \;=\; \int_U f\,d\mu,
\]
with total mass the evidence $\int f\,d\mu$
(\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_bayes_linear_cbv_mass}).
...
$\mathtt{tunit}$-projection of \autoref{lem:cbv-qbs-kit} turns into a
$\mathtt{precone\_scale}$ weight on the returned $\delta_r$.
\end{theorem}

\begin{theorem}[Rejection sampling normalises the Bayes-score
posterior]\label{thm:cbv-reject-bayes}
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_reject_normalises_bayes}\rocqok
\uses{thm:cbv-qbs-bayes, thm:cbv-reject-master}
...
\[
  ...
  \;=\;
  \sem{\mathtt{ex\_bayes\_linear}}(1)(U)
\]
--- the rejection-sampling denotation times the total evidence is
exactly the Bayes-score denotation: $\mathtt{score}$ computes the
...
\autoref{thm:cbv-qbs-bayes} and the master identity of
\autoref{thm:cbv-reject-master}.
\end{theorem}

\begin{theorem}[The QBS marginals]\label{thm:cbv-qbs-marginals}
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_random_constant_cbv_marginal}\rocqok
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_random_constant_cbv_marginal_dirac}\rocqok
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_random_linear_cbv_marginal}\rocqok
\uses{thm:cbv-let-sample, lem:cbv-qbs-kit, thm:cbv-fix-comb}
...
(\autoref{lem:cbv-qbs-kit}) --- at $x = 0$ the marginal is $0$, not
$\mu$. The Dirac instance is
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_random_constant_cbv_marginal_dirac}.
For $\mathtt{ex\_random\_linear}$
($\mathtt{let}\,m = \mathtt{sample}\,\mu\,\mathtt{in}\,
\mathtt{let}\,b = \mathtt{sample}\,\mu\,\mathtt{in}\,
\lambda x.\,m\,x + b$), the marginal at a Dirac test point
...
\]
(\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_random_linear_cbv_marginal},
with the one-sample inner layer
\rocq{Icones.programs.infra.cbv_qbs_marginals.rl_inner_marginal}):
...
twelve surface programs of \texttt{theories/programs/examples.v}
(\rocq{Icones.programs.examples.ex_random_constant},
\rocq{Icones.programs.examples.ex_random_linear},
...
\rocq{Icones.programs.ex_reject_headline.ex_almost_loop_cbv_mass_one}
/ \rocq{Icones.programs.ex_reject_headline.ex_almost_loop_cbv_zero};
and the CBV QBS marginals of \autoref{ssec:cbv-qbs-marginals} ---
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_bayes_linear_cbv_posterior},
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_reject_normalises_bayes},
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_random_constant_cbv_marginal},
\rocq{Icones.programs.infra.cbv_qbs_marginals.ex_random_linear_cbv_marginal}.
...
(\autoref{def:cbv-fix-clause}); $\mathtt{tbool}$ is the \S9.7-style
...
and the closed-form CBV marginal identities for the QBS trio are
proved in \autoref{ssec:cbv-qbs-marginals}. What remains as
follow-up:
\begin{itemize}
  \item $\mathtt{ne\_fix\_mr}$ at $\mathtt{tprod}$ body types still
    routes through the degenerate $\mathtt{Yfix\_fun\_lin}$
    (\rocq{Icones.programs.ppl_cbv.eD_fix_mr_prod_E}); the repair
    needs the Seely transport of
    \rocq{Icones.programs.infra.em_fix_value.fix_comb} along
    $\mathrm{EM\_prod}(\widetilde{\mathord{!}}X,
    \widetilde{\mathord{!}}Y) \cong
    \widetilde{\mathord{!}}(X \otimes Y)$
    (\autoref{def:cbv-fix-clause}).
\end{itemize}
...
$\mathtt{qbs\_normalize}$ / Bayesian-posterior \emph{pass} is also not
