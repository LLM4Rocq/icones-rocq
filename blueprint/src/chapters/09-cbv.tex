...
This is \texttt{theories/programs/ppl.v}, fine-grain Moggi-CBV, axiom-free.
The QBS-paper showcase \texttt{ex\_random\_constant} --- a distribution
over a function space, $\mathtt{P}(\mathbb{R}\to\mathbb{R})$ ---
interprets soundly via this Kleisli-exponential structure.

\paragraph{No further open milestones for higher-order CBV.}
There is no ``value-level application'' that has been suppressed in
favour of a Moggi-style computation: applying an effectful function to
a value gives a computation \emph{by definition} of monadic CBV
semantics. EM(!) being non-closed is structural and irrelevant.
Adequacy, normalization, and full-abstraction are out of scope here,
as are booleans / coproducts (no native sum type in the cones model)
and the QBS-paper's \texttt{qbs\_normalize} / Bayesian-posterior
showcase (orthogonal to the higher-order content shown here).
...
\texttt{theories/homs/cbv_adjunction.v} (the LNL monoidal adjunction
and the \texttt{CBV\_Model} bundle), and \texttt{theories/programs/cbv.v}
