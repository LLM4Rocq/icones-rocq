...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Stable functions and the category \texorpdfstring{$\mathbf{SCones}$}{SCones}}\label{ch:stable}
...
\paragraph{Strategy: from the unit-ball linear setting to general radii.}
The linear development (\autoref{ch:icones}, \autoref{ch:linhom}) only
ever needed $\omega$-continuity on the \emph{unit ball}: for a linear
map every bounded chain rescales into $B_B$, so the unit-ball notion
...
\section{The radius-aware \texorpdfstring{$\omega$}{omega}-continuity foundation}%
...
\section{The local cone \texorpdfstring{$B_x$}{B_x} --- paper \S7.1}\label{sec:local-cone}
...
is precisely where the linear development's simplified norm condition
is satisfiable without weakening any cone axiom. The
$\cnorm{x} \leq 1$ facts needed by the cone development are recovered
from the strict bound by $\texttt{ltW}$.
...
\paragraph{Design note: radius-aware \texorpdfstring{$\omega$}{omega}-continuity.}
The linear development's $\omega$-continuity
(\texttt{is\_omega\_continuous}) restricts both input and image chains
to the unit ball. For linear maps this is harmless --- domain rescaling
brings any bounded chain into $B_P$ --- but a non-linear stable $f$
scaled by $r < 1$ has an image chain reaching norm $1/r > 1$, escaping
the ball, with no pointwise rescaling. The fix is the unit-input
Scott-continuity
...
\subsection{The stable internal hom \texorpdfstring{$B \mathbin{\Rightarrow_s} C$}{B Rightarrow_s C}}
...
\paragraph{Design note: the \texorpdfstring{$0$}{0}-extension carrier.}
...
\subsection{Finite differences and \texorpdfstring{$n$}{n}-increasingness}
...
\section{The category \texorpdfstring{$\mathbf{SCones}$}{SCones} --- paper \S7.4}\label{sec:scones}
...
\paragraph{Design note: morphisms as \texorpdfstring{$0$}{0}-extended maps.}
...
\subsection{The cartesian-closed structure --- paper Theorem 7.32 (closed part)}
\label{sec:scones-ccc}
With the products in hand, the file
\texttt{theories/stable/scones\_ccc.v} completes the cartesian-\emph{closed}
structure of $\mathbf{SCones}$, with $B \mathbin{\Rightarrow_s} C$
(\autoref{thm:stablehom-icone}) as the internal hom. The binary product
$X \mathbin{\&} Y$ is the bool-indexed product
\rocq{Icones.stable.scones_ccc.sprod}, with pairing and projections
$\texttt{sprod\_pair}$ / $\texttt{sprod\_fst}$ / $\texttt{sprod\_snd}$
and the norm identity $\Cuball{X \mathbin{\&} Y} = \Cuball X \times
\Cuball Y$ (\rocq{Icones.stable.scones_ccc.sprod_pair_norm_le1}).

\begin{lemma}[Linear $\times$ totally monotonic $\Rightarrow$ totally monotonic, paper Lemma 7.27]
\label{lem:lin-totmono}
\rocq{Icones.stable.scones_ccc.ev_totmono}\rocqok\uses{def:is-totmono}
A map $f : \Mcca X \times \Cuball{\Mcca Y} \to \Mcca Z$ that is linear in
its first argument and totally monotonic in its second is, restricted to
$\Cuball{\Mcca X} \times \Cuball{\Mcca Y}$, totally monotonic.
\end{lemma}

\begin{proof}
Following the paper. Expanding $\Delta^\varepsilon f$ and using linearity
in the first argument splits the alternating sum into a constant-in-$x$
part $\sum_{I} f(x, y + \sum_{i \in I} v_i)$ --- bounded by the total
monotonicity of $f(x, -)$ at the centre $(x,y)$ --- and a cross part
$\sum_{I} \sum_{j \in I} f(u_j, y + \sum_{i \in I} v_i)$. The cross part
is reindexed by the correspondence $\{I \in \mathit{P}^\varepsilon(n) :
j \in I\} \cong \mathit{P}^\varepsilon(n{-}1)$ (\texttt{inset-corresp},
realised here by a $\texttt{liftI}$ reindexing on $\{j : j \neq i\}$ and
the general transport
\rocq{Icones.stable.scones_ccc.gen_totmono}), and each $j$-summand is
bounded by the total monotonicity of $f(u_j, -)$ at $(u_j, y + v_j)$
over the punctured increment family. Recombining gives
$\Delta^+ f \geq \Delta^- f$ for $n > 0$; the case $n = 0$ is trivial.
\end{proof}

\begin{lemma}[Evaluation, paper Theorem 7.32]\label{lem:ev}
\rocq{Icones.stable.scones_ccc.Ev}\rocqok\uses{lem:lin-totmono, thm:stablehom-icone}
The evaluation $\mathrm{Ev} : (B \mathbin{\Rightarrow_s} C) \mathbin{\&}
B \to C$, $(f, x) \mapsto f(x)$
(\rocq{Icones.stable.scones_ccc.ev_fun}), is a $\mathbf{SCones}$
morphism. Its total monotonicity is \autoref{lem:lin-totmono} instantiated
with the internal hom in the linear slot: evaluation is linear in $f$
because the operations of $B \mathbin{\Rightarrow_s} C$ are pointwise
(\texttt{sh\_addE} / \texttt{sh\_scaleE}), and totally monotonic in $x$
for fixed $f$ because that is $f$'s own total monotonicity. The norm bound
is $\Norm{\mathrm{Ev}} \leq 1$, and measurability follows the paper's
test-pullback argument.
\end{lemma}

\begin{theorem}[$\mathbf{SCones}$ is cartesian closed, paper Theorem 7.32]
\label{thm:scones-ccc}
\rocq{Icones.stable.scones_ccc.SCones_ccc}\rocqok
\uses{lem:ev, thm:scones-prod, thm:stablehom-icone}
$\mathbf{SCones}$ is cartesian closed: the internal hom of $B$ and $C$ is
$B \mathbin{\Rightarrow_s} C$ with evaluation $\mathrm{Ev}$, and every
$h : (D \mathbin{\&} B) \to C$ has a unique transpose
$\Lambda h : D \to (B \mathbin{\Rightarrow_s} C)$
(\rocq{Icones.stable.scones_ccc.curry}, the $0$-extension of
$z \mapsto (x \mapsto h(z,x))$).
\end{theorem}

\begin{proof}
For fixed $z$ with $\cnorm z \leq 1$, $x \mapsto h(z,x)$ is measurable
stable (\rocq{Icones.stable.scones_ccc.curry_app}); that $z \mapsto
\Lambda h(z)$ is itself stable into $B \mathbin{\Rightarrow_s} C$ is the
cone-step \rocq{Icones.stable.scones_ccc.curry_totmono_step}: the
inequality in the internal hom is unfolded by the order characterisation
(\autoref{lem:stable-order}, paper Lemma 7.12 backward) into the
pointwise double-sum over $D$ and $B$ (\autoref{lem:sh-norm}, paper
Lemma 7.12 backward), which the
$(I,J) \mapsto I \cup (J{+}n)$ split (the parity-multiplicative
$\mathit{P}^\pm(n{+}k) \cong \mathit{P}^\pm(n) \times \mathit{P}^\pm(k)$
identity \texttt{Spos\_cat} / \texttt{Sneg\_cat} of \autoref{sec:findiff}),
applied to the interleaved increment family $(w'_l, u'_l)$, reorganises
into $h$'s own total monotonicity over $D \mathbin{\&} B$. The
$\beta$ and $\eta$ laws (\rocq{Icones.stable.scones_ccc.curry_beta} /
\rocq{Icones.stable.scones_ccc.curry_eta}) hold as Leibniz equalities by
a unit-ball case split, all morphisms being $0$-extended off the ball.
The whole structure is packaged as
\rocq{Icones.stable.scones_ccc.SCones_CCC}.
\end{proof}

\section{Least fixpoints --- paper \S9.2}\label{sec:yfix}

The least-fixpoint operators of paper \S9.2 live in
\texttt{theories/stable/fixpoint.v}: they are pure $\mathbf{SCones}$
content (the unit-ball $\omega$-cpo of a cone plus the
cartesian-closed structure of \autoref{sec:scones-ccc}), and they are
the recursion engine consumed downstream by the CBV value-fixpoint of
Chapter~\ref{ch:cbv}.

\begin{lemma}[The fixpoint operators $\mathrm{lfp}$ and
$\mathrm{Yfix}$, paper \S9.2]\label{lem:yfix}
\rocq{Icones.stable.fixpoint.lfp}\rocqok
\rocq{Icones.stable.fixpoint.Yfix}\rocqok
\rocq{Icones.stable.fixpoint.Yfix_fix}\rocqok
\uses{def:stablehom, thm:scones-ccc}
For an integrable cone $B$, every monotone ball-preserving
$f : B \to B$ has a least fixpoint
$\mathrm{lfp}\,f = \sup_n f^n(0)$, the supremum of the Kleene chain on
the unit ball (\rocq{Icones.stable.fixpoint.lfp}, with the fixpoint
equation \rocq{Icones.stable.fixpoint.lfp_fixpoint} under Scott
continuity on the ball). Packaged parametrically, the assignment
$f \mapsto \mathrm{lfp}\,f$ is itself a $\mathbf{SCones}$ morphism
\[
  \mathrm{Yfix} : (B \mathbin{\Rightarrow_s} B) \longrightarrow B
\]
(\rocq{Icones.stable.fixpoint.Yfix}, built through the $Z$-combinator
on the internal hom and the cartesian-closed structure of
\autoref{thm:scones-ccc}), satisfying the fixpoint law
$f(\mathrm{Yfix}\,f) = \mathrm{Yfix}\,f$ for every unit-ball $f$
(\rocq{Icones.stable.fixpoint.Yfix_fix}). Recursion at any
$\mathbf{SCones}$ function space is therefore available
unconditionally --- a single application of $\mathrm{Yfix}$ at the
internal hom.
\end{lemma}
...
(Theorem 7.30, \autoref{sec:findiff}). \S7.4 is now fully delivered,
axiom-clean, on the $0$-extension morphism carrier: the category
$\mathbf{SCones}$, the dereliction $\mathrm{Der}$, the products with
their universal property (\autoref{sec:scones}), and the
cartesian-\emph{closed} structure --- the evaluation $\mathrm{Ev}$,
currying, the $\beta$/$\eta$ laws, and Theorem 7.32 in full
(\autoref{sec:scones-ccc}, \rocq{Icones.stable.scones_ccc.SCones_ccc}).
The route to the exponential $\mathord{!}$ is then via the $E \dashv
\mathrm{Der}$ adjunction, obtained from the Special Adjoint Functor
Theorem once $\mathrm{Der}$ is shown limit-preserving (Theorem 7.34,
\rocq{Icones.stable.der_continuous.der_preserves_limits}); this is now
\textbf{discharged}, so the $\mathord{!}$ comonad
(Chapter~\ref{ch:exponential}) is axiom-free.
