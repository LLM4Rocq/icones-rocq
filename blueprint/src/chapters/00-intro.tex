...
structure} of Chapter~\ref{ch:cbv} is also built and axiom-free, with
three honest caveats stated there (the cartesian structure lives on the
rich subcategory of comonoidal coalgebras, not the full
$\mathrm{EM}(\mathord{!})$; comonoidality is not proved closed under the
product; and no CBV/CBPV calculus is yet interpreted). What remains open
otherwise is genuinely beyond \S9: paper \S8 (analytic functions, the
category $\mathrm{ACONES}$), the \S9 Eilenberg–Moore full-subcategory
theorem ($\mathrm{ARCAT}$ on Polish/standard-Borel spaces
$\hookrightarrow \mathrm{EM}(\mathord{!})$, needing a
Polish/standard-Borel layer not yet formalised) and the PCS embedding
(\S10).
