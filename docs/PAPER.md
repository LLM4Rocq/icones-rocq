...
```coq
(* theories/cones/precone.v — the instance-facing factory; the
   underlying mixin [POrder_isPrecone] additionally pins the boolean
   [Order.POrder] on [precone_display] to the cone order via
   [precone_le_def : (x <= y)%O = `[< exists z, y = x + z >]] *)
...
> **Difference — the cone order is a mathcomp `Order.POrder` on the dedicated display `precone_display`, obtained by classical reflection.** The cone order $x_1\le x_2\iff\exists z\,.\,x_2=x_1+z$ is in general undecidable, so the boolean order is defined classically: `(x <= y)%O = `[< exists z, y = x + z >]` (boolp's `asbool`, pinned by the mixin field `precone_le_def`). This puts the whole mathcomp order theory (`lexx`, `le_trans`, `le_anti`, …) at the disposal of every `preconeType`. The `Prop`-valued definitional form is kept as `precone_le` (notation `x <=p y` in `precone_scope`), connected to the boolean order by the reflect view `precone_leP` (and `precone_leE`); all its historical lemmas — reflexivity / transitivity / antisymmetry and the monotonicity / cancellation rewriting lemmas (`precone_add_le_r`, `precone_add_le_l`, `precone_le_addlI`, `precone_le0`, `precone_scale_le`) — live on unchanged in `Section PreconeOrder` of `precone.v`, alongside the chain bridges `precone_chain_homo` / `homo_precone_chain` between the step form `forall n, u n <=p u n.+1` and the mathcomp `{homo}` form. Antisymmetry is still *derived* from (Cancel) and (Pos) exactly as in the paper's §2.1 — inside the `isPrecone` factory, where it feeds `Order.Le_isPOrder` — and it is what makes the difference $x_2-x_1$ unique. Instances go through the `isPrecone` factory, whose field list is exactly Def 2.1; the only new obligation is a `Choice` structure on the carrier (classical `gen_eqMixin`/`gen_choiceMixin` where none is canonical). One consequence of the factory now *installing an order instance* deserves to be stated as a rule: **the carrier of an `isPrecone` instance must be a key of its own** — a `Record`, an `Inductive`, or a `sig`, never a bare alias of a type that already carries a mathcomp order. Registering `Order.POrder precone_display` on someone else's key does not make HB complain (mathcomp's own order sits on a different key), but the cone order then *wins* canonical resolution for every user-written occurrence of that type and silently disables the library's order theory on it. Every carrier of the tower obeys the rule: `ConeBot.T`, `ConeOne.T`, `bool_cone_car`, `cone_one_car`, `path_car`, `linhom_car`, `cones_prod_car`, `cones_eq_car`, `alpha_rescale_car`, and `local_cone`'s `sig`.
...
> **Difference — none, anymore: (Normc) is the paper's pure existence statement.** The mixin field `cone_normc` says exactly "every increasing chain in the unit ball has a least upper bound, of norm $\leq 1$", with the chain in mathcomp `{homo}` form. The supremum itself is the *total* operator `cone_sup : (nat -> P) -> P`, defined classically (via `pselect`/`cid` on the least-upper-bound predicate `cone_lub`) over any `preconeType` — garbage off-spec, exactly like mathcomp-analysis' `lim`/`sup`/`integral`. Chain-ness and boundedness enter the theory as *lemma hypotheses* (`cone_lub_sup`, `cone_norm_sup_ball`, and the general-radius family of `omega_general.v`), never as operator arguments. The historical witness-style operator `cone_sup_ball u uch ub1` survives as a wrapper `:= cone_sup u` with its three characterising lemmas (`cone_sup_ball_ub` / `cone_sup_ball_lub` / `cone_sup_ball_norm`) in their historical statements; instances are built through the `Cone_ofWitnessSup` factory, whose field list is exactly the historical one.
...
>
> One consequence of routing instances through the `Cone_ofWitnessSup` factory is worth recording here, because it is the only place a call site can feel it. For a factory-built carrier the `isCone` field `cone_norm` is no longer *syntactically* the carrier's own norm function, so Rocq can no longer invert the projection by canonical-structure resolution: `exact: cone_norm_ge0` does **not** close a goal phrased with the concrete head, e.g. `0 <= fmeas_norm µ`, although the two remain *convertible* — `@cone_norm_ge0 _ (fmeas _ _) µ` typechecks against that goal, and `exact: lexx` still proves `fmeas_norm µ <= cone_norm µ`. Such goals are therefore discharged with the carrier's own lemma (`fmeas_norm_ge0` in `Skern_comp_norm_le1` of `theories/kernels/skern.v` and in `Skern_to_ICones_mor_norm_le1` of `theories/kernels/kernel_embedding.v`), which is the more direct statement anyway. Nothing about the *meaning* of any norm lemma changes; only the unifier's keying does.

> **Design note — why the sup is a *total, spec-defined* operator.** The mixin axiom `cone_normc` is a pure existence statement, and the operator `cone_sup` extracts the least upper bound *once*, by `cid` on the lub predicate; off-spec it returns junk (`0`), exactly in the style of mathcomp-analysis' `lim`/`sup`/`integral`. The §3–§7 proofs still get the sup *as a function of the chain* — `cone_sup u` mentions no proofs at all, so two suprema of the same chain are *syntactically* equal, and proof-irrelevance juggling on chain/bound witnesses disappears. Rewriting between a supremum and a concrete construction goes through the identification lemmas: `cone_supE` (any lub *is* `cone_sup`), `cone_sup_ballE` (its `Prop`-shaped form for the historical wrapper), and per-instance corollaries such as `cones_prod_cone_sup_ballE` / `cones_eq_cone_sup_ballE` (products and equalisers, `cone_cat.v`) and `fmeas_cone_sup_ballE` / `path_cone_sup_ballE` / `linhom_cone_sup_ballE`, `c1_cone_sup_ballE` (the unit cone, `icones/examples_icone.v`) and `αB_cone_sup_ballE` (the Def 3.15 rescaling, `mcones/mcone_cat.v`) and `sh_cone_sup_ballE` (the stable hom of `stable/stablehom.v`, §7.2) — plus, on the PPL side, `bc_cone_sup_ballE` for the 2-point cone of `icones/bool_cone.v` — these replace the definitional reduction of the historical mixin-projection encoding. Where the two suprema are of *pointwise-equal* chains rather than of a chain and a concrete construction, `eq_cone_sup` is the tool, and it collapses several pre-rework antisymmetry arguments to one line (`cones_id_continuous`, `cones_eq_sup_P_in_E`, `invf_omega_continuous`, the three PPL chain identifications `Yfix_kleeneE` / `fix_value_E` / `fix_coalg_simpl` of `theories/programs/infra/em_fix_value.v`, and — in the §9/CBV leaves — `linhom_to_stablehom_continuous` of `theories/cbv/diag_bilinear_tensor.v`, whose diagonal step is now `rewrite !cone_sup_ball_supE; apply: eq_cone_sup` after the two per-instance rewrites `linhom_cone_sup_ballE` and `sh_cone_sup_ballE`; in the same file `meas_stable_id`'s $\omega$-continuity obligation and *both* radius-$M$ component steps of `spair_meas_stable` became `by []` — the latter used to be two five-line `precone_le_anti` chases between the `cone_sup_at` of a chain and the `cone_sup_at` of its projection, which are now literally the same term; and the two rewrites replace the four `rewrite -[cone_sup_ball … : _]/(…_sup_ball …)` fold steps of `theories/exp/stab_lin_swap.v`); the same collapse happens wherever a sup is transported through a *faithful projection* — in `theories/homs/limpl_continuous.v` `corestr_continuous` is now the single rewrite `cones_eq_cone_sup_ballE` (the equaliser sup is computed through `cones_eq_val`), and the `prodComp` step of `limpl_prod_inv_map_continuous` is `exact: cones_proj_continuous` (the projection's own $\omega$-continuity now closes it definitionally); both used to end in a two-sided `precone_le_anti` argument. Because `cone_sup` is characterised by its spec rather than a construction, the general-radius theory of `theories/cones/omega_general.v` (`cone_normc_at`, `cone_sup_ub`, `cone_sup_le_ub`, `cone_norm_sup`, `cone_supD`, `cone_sup_swap`) is about the *same* operator: an $M$-bounded chain's `cone_sup u` is its lub simply because the lub exists, and the historical `cone_sup_at` becomes a phantom wrapper whose radius-independence (`cone_sup_at_indep`) and radius-1 agreement (`cone_sup_at_ball`) are definitional. The §2 continuity lemmas remain sup *identities* (e.g. $\sup_n(u_n+y)=(\sup_n u_n)+y$).
...
> **Difference.** `is_omega_continuous` applies only to chains that are increasing *and* in the unit ball, so a fully-quantified two-argument statement must supply those two side conditions for every intermediate family — each row, each column, the row-suprema, the diagonal — and stays hypothesis-heavy (the note preceding `Section Lemma219` in `cone_cat.v` spells the shape out). What the total-operator encoding of the supremum *did* remove is the other half of the old load: suprema no longer carry their chain/bound proofs, so two suprema of one chain are equal on the nose and the identification steps the pre-rework proofs needed are gone. `diagonal_collapse` is the load-bearing pointwise form actually consumed downstream: given a jointly monotone doubly-indexed family $a$, the row-sup $\sup_k a(n,k)$ is $\leq$ the diagonal sup $\sup_k a(k,k)$. The double-sup expansion ($f(\sup u)(\sup v)=\sup_n\sup_k f(u_n)(v_k)$) then follows mechanically from separate continuity.
...
> **Difference.** The mechanisation establishes *existence* of the transported **precone** structure only — `transport_isPrecone : isPrecone R S`, built from `trans_add`/`trans_scale`/`trans_norm` and the twelve `trans_*` axiom lemmas. Packaging the full `HB.instance` of the cone (with the norm axioms) is deferred as a downstream task and re-stated per target file when needed concretely (see the note preceding `transport_isPrecone` in `cone_cat.v`); uniqueness is exactly the observation that every transported axiom is determined by $f$. Since the cone order is now a `Order.POrder`, the transport section declares its target carrier through `HB.declare Context (S : Type) of hasDecEq S & hasChoice S` — the transported `isPrecone` factory sits above `Choice`, so a bare `Type` no longer suffices.
...
> **Difference — the `Module` wrappers, the fresh carrier keys, and the LL-transposed names.** Each example lives in its own `Module`, and the two names read backwards unless one keeps the LL dictionary in mind: `ConeBot.T` carries $\top$ (the *zero-dimensional* cone) and `ConeOne.T` carries $\mathbf{1}=\bot$, so grepping for "bot" finds the wrong module. **Both carriers are fresh types**, declared for this purpose: $\top$ is `Inductive T : Type := zero_top`, $\bot$ is the one-field `Record T := MkOne { one_val : {nonneg R} }`. Freshness is not cosmetic. HB attaches at most one canonical `isPrecone` / `isCone` instance per carrier type, so reusing a type the library already uses would clash; and — the sharper reason on the $\bot$ side — the `isPrecone` factory now installs an `Order.POrder` instance (on `precone_display`) on its carrier. Installed on the bare `{nonneg R}`, i.e. on mathcomp's own `Itv.nonneg` key, that instance *wins* over `Num`'s order for every user-written `{nonneg R}`: `(x <= y)%O` on non-negative reals silently stops being the `Num` order and the whole `Num.Theory` toolkit (`num_le` & co) stops applying downstream of an `Import ConeOne`. The two relations do coincide extensionally — that is exactly `precone_leE` — so nothing was ever unsound, but the wrapper is what keeps the two *instances* apart, and it is the same discipline every other carrier of the tower follows (`bool_cone_car`, `cone_one_car`, `path_car`, `linhom_car`, `cones_prod_car`, `cones_eq_car`, `alpha_rescale_car`). What the `Module` wrapper buys on top is name hygiene: both modules declare `add`, `scale`, `norm` and `sup_ball`, so without it the second set would shadow the first (as it is, an unqualified `norm` in `examples_cone.v` is `ConeBot.norm`). The rewriting equations `precone_add_E` and `precone_leE`, which identify the abstract cone operations with the underlying `{nonneg R}` arithmetic and order *through* the `one_val` projection, are what downstream proofs actually compute with. The `Ar`-indexed re-wrapping `cone_one_car` of `icones/examples_icone.v` is the measurable-cone counterpart of the same pattern.
...
(* Module ConeOne, Variable R : realType: the one-dimensional cone ⊥ = 1.
   Its carrier is a thin Record wrapping {nonneg R}, its norm the [%:num]
   of the wrapped value, and its precone operations the {nonneg R} ring
   operations: *)
...
| Thm 4.19 | $\mathbf{ICones}$ is complete with $1$ a coseparator; therefore every limit-preserving functor $\mathbf{ICones}\to\mathcal{C}$ has a left adjoint (by SAFT). | The completeness data (products, equalisers, the coseparator) is in `theories/icones/icone_cat.v`; the bespoke **SAFT engine** is `theories/homs/representable.v` (`wi_obj`, `wi_med`, `is_icones_left_adjoint`) — see *Beyond the paper* below |
...
| Thm 5.13 | Norm identity for pure tensors: $\lVert x\otimes y\rVert=\lVert x\rVert\,\lVert y\rVert$. | `tensor_norm_le` ($\le$) + the $\ge$ direction via Prop 3.11 — `theories/homs/tensor.v`, `theories/homs/tensor_iso.v` |
...
| Rem 5.1 | The tensor object is given by SAFT, without an explicit carrier. | The paper's invocation of SAFT is *mechanised* concretely in `theories/homs/representable.v` + `theories/homs/tensor_construct.v` — see *Beyond the paper* below |
...
| Lem 7.1 | The gauge $N$ is a norm on the local cone $P=B_x$, and $P$ is a cone whose $0$, operations and order are those of $\mathcal{M}B$. | `local_cone` (`Cone_ofWitnessSup.Build`), `lc_normh`, `lc_normt` — `theories/stable/local_cone.v` |
...
The cone mixin's (Normc) axiom asserts completeness only on the unit ball; §7 needs the same calculus at an arbitrary radius, so `theories/cones/omega_general.v` builds it — for the *same* total operator `cone_sup` of `cone.v`. The engine is `cone_normc_at`: an $M$-bounded $\leq$-increasing chain has a least upper bound of norm $\leq M$ (for $M>0$ by rescaling into $\mathcal{B}P$ and applying `cone_normc`; the $M=0$ chain is constantly $0$), whence `chain_has_cone_sup`, and the primary laws `cone_sup_ub`, `cone_sup_le_ub`, `cone_norm_sup`, `le_cone_sup` — all with the chain in `{homo}` form and the radius a plain real hypothesis. The historical `cone_sup_at M u` survives as a phantom wrapper `:= cone_sup u` with its historical laws `cone_sup_at_ub` / `cone_sup_at_lub` / `cone_sup_at_norm`; `cone_sup_at_ball` (radius-1 agreement with `cone_sup_ball`) and `cone_sup_at_indep` (radius-independence) are now *definitional*. `is_scott_continuous f` is commutation of $f$ with `cone_sup_at` at any input radius $M$ and image radius $M_f$, and `linear_scott_of_omega` is the reusable bridge — a linear, unit-ball $\omega$-continuous map is Scott-continuous — proved by scaling the lub through `cone_lub_scale` at the single common radius $K:=\max(M,M_f)$ and identifying suprema with `cone_supE`. The corollaries `addr_scott_continuous` / `addl_scott_continuous`, `scaler_scott_continuous` and `comp_scott_continuous` record that translation, scaling and composition are Scott-continuous; the scaling one is what makes `stable_scale` (Lem 7.11) go through for *every* $r$. The same file carries the *consolidated sup-calculus* — chain monotonicity `precone_chain_le`, proof-irrelevance `cone_sup_ball_irr` (kept as a name its consumers cite, but now *definitional*: `Proof. by []. Qed.`), the diagonal sup-addition identity `cone_sup_at_addD`, the iterated-(Normt) bound `cone_norm_sum`, the finite-sum supremum `cone_sum_sup` / `cone_sum_sup_eq` and the scaled chain `scale_chain_sup` — each proved once at a general radius, with the radius-$1$ corollaries in the shape `linhom.v`, `totmono.v`, `stablehom.v`, `findiff.v` and `scones_ccc.v` consume.

On the §7 side the rework shows up in three ways. *Instances:* the three cone carriers built in this chapter — `local_cone x` (Lem 7.1), `stablehom B C` (Lem 7.14) and `SnB B n` (Lem 7.20–7.25) — keep their concrete witness-style sup constructions and register through the `Cone_ofWitnessSup` factory; each carrier additionally gets the classical `gen_eqMixin`/`gen_choiceMixin` pair the precone partial order now requires. *Proof-irrelevance:* the two `congr cone_sup_ball; exact: Prop_irrelevance` / `congr cone_sup_at; exact: Prop_irrelevance` steps (`stable_scale` in `totmono.v`, `sh_sup_scott` in `stablehom.v`) are gone — the witnesses are phantom, so both sides are the same `cone_sup`. *Identification lemmas:* where a proof used to rely on the mixin projection *reducing* to the instance's construction, it is now one rewrite — `sh_cone_sup_ballE` in `sh_test_cont` (`stablehom.v`) and in `ev_scott` (`scones_ccc.v`), `cones_prod_cone_sup_ballE` in `sprod_fst_sup` / `sprod_snd_sup` / `sprod_pair_sup` / `sprod_pair_sup_l` (`scones_ccc.v`), `cones_eq_cone_sup_ballE` in `der_incl_omega` (`der_continuous.v`, which thereby collapses from a two-sided `precone_le_anti` to a single rewrite). The only §7 proof that had to be *redone* rather than rewritten is `test_of_sup_at` (`stablehom.v`): it consumed the historical *definition* `cone_sup_at ach aubM Mpos = M *: cone_sup_ball …`, which is now derived on the spot from `cone_supE` applied to `cone_lub_scale`. `lfp` / `Yfix` (`fixpoint.v`) compile untouched: `lfp f f_incr f_ball` is literally the total `cone_sup (kleene f)`, its witnesses phantom.
...
> **Difference.** The paper states $\omega$-continuity for an $\omega$-closed subset $A\subseteq P$ and *arbitrary bounded* increasing sequences, so no radius ever appears in it; the mechanised (Normc) axiom asserts completeness on the *unit ball* only (`cone_normc`, see the Def 2.2 note). A radius-aware layer therefore has to be built before §7 can start — though with the total `cone_sup` this is a family of *lemmas about the same operator* rather than a second operator. *Why it cannot be avoided:* for a **linear** map every bounded chain rescales into $\mathcal{B}P$ and back, so the unit-ball notion loses nothing — which is why §§2–6 never need this file. A **non-linear** stable $f$ scaled by $r<1$ has an image chain of norm up to $1/r>1$, which leaves the ball and cannot be rescaled back pointwise. Hence `is_stable` uses the unit-*input* form `is_scott_continuous_unit` (input chain in $\mathcal{B}P$, image supremum at any radius — `theories/stable/totmono.v`), which is exactly `is_scott_continuous` specialised to input radius $1$ through `cone_sup_at_ball`. The fully general `is_scott_continuous` would be *too strong* for stable maps: they are monotone only on $\mathcal{B}P$, so for an input chain leaving the ball the image chain need not even be increasing.
...
The local cone $B_x$ of $B$ at $x\in\mathcal{B}\mathcal{M}B$ is the set of directions $u$ with $x+\lambda u\in\mathcal{B}\mathcal{M}B$ for some $\lambda>0$; equipped with the gauge norm $N(u)=(\sup\{\lambda>0\mid x+\lambda u\in\mathcal{B}\mathcal{M}B\})^{-1}$ it is a cone whose $0$, algebraic operations and order are inherited from $\mathcal{M}B$. The algebraic half is the `isPrecone` instance — the factory of `precone.v`, whose only new obligation is a `Choice` structure on the `sig` carrier, supplied classically by `gen_eqMixin`/`gen_choiceMixin` since `localP` is `Prop`-valued — the norm laws are `lc_normh` (homogeneity) and `lc_normt` (sub-additivity), and the whole is assembled into the cone HB instance on `local_cone x` through the `Cone_ofWitnessSup` factory (`cone.v`), which keeps the historical field list: the witness-style operator `lc_sup_ball` and its three characterising lemmas feed it unchanged. Two supporting facts carry the norm: `gauge_set_ub` bounds the admissible-scale set (for $u\neq0$ every admissible $s$ satisfies $s\le\lVert u\rVert_B^{-1}$, so the supremum defining $N$ exists), and `lc_val_norm_le` is the paper's $\lVert u\rVert_B\le\lVert u\rVert_{B_x}$ — the inequality that later lets every $B$-test pull back to a $B_x$-test. (Normc) is `lc_sup_ball_translate`: the $B_x$-supremum of a chain is read off the $B$-supremum of the *translated* chain $n\mapsto x+u_n$.
...
(* Classical Equality/Choice on the sig carrier: mathcomp partial
   orders sit above choiceType, and localP is Prop-valued. *)
...
(* Paper Lemma 7.1: B_x is a cone, norm the gauge lc_norm.  Built
   through the historical-interface factory of cone.v. *)
...
(* Classical Equality/Choice on the record carrier: mathcomp partial
   orders — hence the precone order — sit above choiceType. *)
HB.instance Definition _ := gen_eqMixin (stablehom B C).
HB.instance Definition _ := gen_choiceMixin (stablehom B C).
...
(* With (Normh)/(Normz)/(Normt)/(Normp) and (Normc) above:
   [stablehom B C : coneType R] — the cone B ⇒ₛ C.  Registered through
   the historical-interface factory of cone.v. *)
HB.instance Definition _ :=
  @Cone_ofWitnessSup.Build R (stablehom B C) (* sh_norm + the five axioms *).

(* The abstract cone_sup_ball IS the concrete sh_sup — definitional
   before the total-cone_sup rework, an equation (lub-uniqueness) now. *)
Lemma sh_cone_sup_ballE u uch ub1 : cone_sup_ball u uch ub1 = sh_sup uch ub1.
```
...
| Lem 9.3 | The exponential functor on a promoted point: $({!f})(x^!)=f(x)^!$. | `bang_fmap_prom` (the computation behind `bang_fmap_id` / `bang_fmap_comp` functoriality and `Coalg_coassoc`) — `theories/exp/bang.v`, `theories/exp/coalgebra.v` |
