...
chapter states what can then be proved about `eD`: the β-rule, the
...
named_expr G t1 -> named_expr G t2`, not Moggi fine-grain — and `ne_lam`
...
the comonoid-diagonal recipe `δ_Γ ; (id_Γ ⊗ ⟦M⟧) ; ⟦K⟧`.
...
the unit-ball discipline of `linhom_icones`, and what it rules out —
...
boolean twin of `score_lift`. The witness layout of `ne_bernoulli_f`
...
(pushforward), whose operator norm is already `≤ 1`.
...
`f̂ = R_to_carrier ∘ f ∘ carrier_to_R`, with two load-bearing laws:
`meas_lift_dirac` (on a point mass, application — `δ_r ↦ δ_{f r}`) and
`meas_lift_mass` (the pushforward preserves total mass).

> The CBV clause is `eD_meas_E` (`theories/programs/ppl_cbv.v`):
> `⟦Meas f e⟧ = meas_lift ∘ ⟦e⟧` — the `FMeas`-functorial mirror of the
> `ne_score` clause, with a functor action where scoring has a path
> lift. This is the one effect-shaped constructor that needs no bound
...
`⟦Gaussian(e1,e2)⟧ = δ_Γ ; (⟦e1⟧ ⊗ ⟦e2⟧) ; kernel_lift2 gaussian_kernel`.
...
(`pk_meas`: `x ↦ k(x)(U)` is measurable for every measurable `U`) and
...
to the semantic lift `kernel_lift k : FMeas X ⊸ FMeas Y`,
`ν ↦ ∫ k(x) ν(dx)` (Pettis integral).
...
(`kernel_lift k δ_x = k(x)`). Two-argument kernels go through
`kernel_lift2 k := kernel_lift k ∘ fmeas_lax` — the tensored argument
...
`dirac_kernel_lift_id`), `gaussian_kernel` (`(m,s) ↦ normal_prob m s`
transported along the carrier cast) and `uniform_kernel`
(`(a,b) ↦ uniform_prob` for `a < b`, else `δ_a`). Family measurability
...
> the Dirac `δ_m` — the degenerate weak limit of `N(m, s)` as `s → 0` —
> because mathcomp-analysis' own `normal_prob m 0` is a junk
> uniform-`[0,1]` placeholder (its `normal_pdf` falls back to
> `uniform_pdf 0 1` at `s = 0`), not a meaningful distribution; `s ≠ 0`,
...
| Strict positivity and the comparison coin | The indicator `\1_(0,∞)` as a `tProb`-producing primitive, and the comparison coin built on the sign of a difference | `pgt0`, `gt0_ind`, `negr` — theories/programs/ppl.v |
| Test functions | The bundled measurable `[0,1]`-valued test function `testfn` and the abstract coin `test f e` that evaluates it at a runtime value | `testfn`, `ptest`, `ex_condition_comb` — theories/programs/ppl.v; theories/programs/examples.v |
...
`h : R → R` through the inclusion (`h = ι ∘ po_into h`, the computation
...
score over it integrates the inclusion `ι` — the mean of the supported
measure.
...
map `sigmoid`; `Gausslik e { s , y }` pushes it through the
...
`gt0_ind = \1_(0,∞)`, landing in `tProb`. It is the primitive behind the
...
measures *against*, with `∫ f dµ` the test pairing that is literally the
content of the conditioning and rejection headline theorems
(`∫ f dµ · ν(U) = ∫_U f dµ`). `test f e` evaluates the test at the
runtime value of `e`, landing in `tProb`: the object-language
counterpart of the integration pairing.
...
measurability and `0 ≤ f ≤ 1` witnesses (`test_meas` / `test_ge0` /
...
reads `Bernoulli (test f #"x")` or `Score (test f #"m")`. The Pyro-style soft conditioning operator
`Condition { f } M` applies a bundled test function to a closed model
`M` and is internally `Score (test f #"x")`
(`theories/programs/examples.v`, `ex_condition_comb`).

> Contrast `observe Dist y`, the special case where the test comes from a
...
> `f`, which is exactly why the abstract `test f` form exists; see the
> [Examples tab](../../examples/index.html) for the conditioning law and
> the rejection-sampling equivalence.
...
its unit-ball witness `pm_ball`, which is what lets `sample m` take a
single argument; `prob_pmeas` transports any mathcomp-analysis
...
> chapter that leaves `tProb`, and it is the inclusion `ι` itself read as
> a term constructor.
...
`N(value(e), s)`, normalised by the distribution's intrinsic peak
`normal_peak s` so that the weight is a legal `[0,1]` density — no
user-supplied envelope. The denotation lemma is `observe_gauss_E`
(`theories/programs/ppl_cbv.v`): at a setlike Dirac environment the
trace is weighted by `normal_pdf μ s y / normal_peak s` at the runtime
mean `μ`.
...
`Bernoulli` / `Score` forms. Both recursive bindings are the derived
...
`µ` to `f · µ`, of mass `∫ f dµ`. As a morphism its operator norm is
...
exactly this. `N(μ, σ)` peaks at its intrinsic peak
`normal_peak σ = 1/(σ√(2π))`; the *unnormalised* pdf crosses `1` as soon
as `σ < 1/√(2π) ≈ 0.399`, so it is not directly a `[0,1]` weight. But
the peak is a finite bound, and dividing by it gives the legal weight
`gauss_obs_density σ y = normal_pdf μ σ y / normal_peak σ ∈ [0,1]` — a
...
| Construction | Statement | Rocq |
|---|---|---|
| The semantic target: EM cartesian structure | Every `!`-coalgebra carries a commutative comonoid `(δ, ε)`, and these comonoids make `(EM(!), ⊗, 1)` cartesian: pairing, projections and the terminal morphism are definable from `(δ, ε)` alone. | `coalg_d`, `coalg_e`, `em_pair_mor` — theories/homs/em_cartesian.v; theories/programs/infra/cbv_adjunction.v |
...
| The value-dependent Bernoulli lift | The semantics of `ne_bernoulli_f`: a measure `µ` on the reals is sent to the 2-point sub-probability `(∫ f dµ, ∫ (1 − f) dµ)`. | `bern_lift`, `bern_lift_mass`, `bern_lift_P` — theories/programs/ppl.v |
...
The unbundled cartesian-η identity `em_pair_mor_proj_id` is exposed
...
> independent product `µ ⊗ µ` a cofree `bang_cofree (bool_cone_car Ar)`
...
> used: `eD_cbv` is `icones_hom`-valued, which keeps the norm-`≤1`
...
> paper reader expecting a Kleisli-style `⟦Γ⟧ → T⟦τ⟧` should note
...
> of `Γ` that produced the boolean: this is what makes
...
itself in the recursion chapter.
...
> pack as the displayed definition of `⟦−⟧`, clause by clause.
...
`x_{n+1} = der (F (x_n !))` is the *interleaved* Kleene chain. It lives in
...
`⟦fix s.body⟧ = fix_comb ∘ ⟦λs.body⟧` — directly at function body types, and
...
`theories/programs/infra/em_fix.v`'s linear step `prev ↦ M ∘ (id ⊗ prev) ∘ δ`
...
re-enters the body as a promoted value — the `prom ∘ der` sandwich is what
makes the chain productive for *any* unit-ball body, linear or not (the step
`x ↦ der (F (x!))` is monotone because `prom` is totally monotone and `F`,
`der` are linear). Coalgebra-morphism-ness of the combinator comes for free:
the value map `F ↦ sup_n x_n` is built from existing `SCones` morphisms,
...
| The naive linear Kleene step | The linear step `prev ↦ M ∘ (id ⊗ prev) ∘ diag` and the linhom least-fixpoint core it used to be iterated in. | `Phi_fun`, `linhom_lfp`, `linhom_lfp_fixpoint` — theories/programs/infra/em_fix.v |
...
| The seeded Kleene core | The Kleene chain from an arbitrary seed `b0` with `b0 ≤ f b0`, and its fixpoint equation. | `kleene_from`, `lfp_from`, `lfp_from_fixpoint` — theories/programs/infra/em_fix_value.v |
| The interleaved chain and the value map | The chain `x_{n+1} = der (F (x_n !))` and the stable map sending a body to its supremum. | `fix_chain`, `fix_value`, `fix_value_E` — theories/programs/infra/em_fix_value.v |
...
| Coalgebraic bodies: the literal chain | On a coalgebra-morphism body the interleaved chain coincides with the literal chain `n ↦ Fⁿ(0!)`. | `fix_setlike_prom`, `fix_coalg_simpl`, `fix_unfold_coalg` — theories/programs/infra/em_fix_value.v |
...
— the `id_Γ ⊗ 0` tensor vanishes by bilinearity, then post-composition by the
...
> diverging-function value, which has `e_bang`-mass one and is therefore not
> even close to `precone_zero` in the cone order. A least fixpoint computed
...
denotation `prev : Γ ⊸ B` to `M ∘ (id_Γ ⊗ prev) ∘ diag` — "run the body with
`prev` bound to the recursive variable" — extended trivially off the unit ball.
Its zero-seeded Kleene supremum on the unit-ball ω-CPO of `linhom_car Ar Γ B`
...
Every inhabitant of the internal hom `linhom_car Ar B C` is a stable and
...
> `F ↦ der ∘ F`, feeding `fix_value` — it is the first ingredient of the value
...
For any measurable-stable `K : G → EA` and any bilinear `Φ : G ⊗ EA → EB` (an
`icones_hom` out of the tensor), the diagonal evaluation `g ↦ Φ(g ⊗ K g)` is
...
The generic Kleene chain `n ↦ fⁿ(b0)` for an *arbitrary* seed `b0` with
`b0 ≤ f b0` replacing the `precone_le0` base of the zero-seeded chains, stated
on a bare `coneType` so that it covers both cone-point chains (the literal
chain `Fⁿ(0!)` of `fix_coalg_simpl`) and linhom-level chains. The fixpoint
equation holds under ω-continuity of the step on the ball.
...
> The whole point of the seed hypothesis `b0 ≤ f b0` is that the zero-seeded
...
Fix `A` and write `!A := Bang Ar A`, `LL := !A ⊸ !A`. The interleaved chain of
a body `F : LL` is `x_0 = 0 : A`, `x_{n+1} = der (F (x_n !))`: each iterate is
...
assignment `F ↦ x_n`: `prom` is totally monotone (it is the underlying map of
...
The assignment `F ↦ sup_n x_n` is then packaged as a *stable* map — obligation
(a) of the construction. It is the composite of existing `SCones` morphisms (so
total monotonicity, ω-continuity and path-measurability come for free): the
linear-to-stable lift `linhom_to_stablehom` of the post-action `F ↦ der ∘ F`
(this is `fix_lts`), curried through the CCC of `theories/stable/scones_ccc.v`
...
`f` is identified with the plain Kleene supremum `sup_n fⁿ(0)` by
...
> The `prom ∘ der` sandwich is the whole repair: it makes the chain productive
...
A stable map `(!A ⊸ !A) → A` *is* a linear map `!(!A ⊸ !A) ⊸ A` via the SAFT
...
`!A → !A`, the interleaved chain coincides with the *literal* Kleene chain
`n ↦ Fⁿ(0!)` in `!A`, seeded at the diverging value `0!` — the naive iteration
one would have written by hand. The bridge is `fix_setlike_prom`: a
coalgebraic body sends promoted points to promoted points
(`F(y!) = (der (F (y!)))!`, read off the coalgebra-morphism square at `y!` plus
...
> Note the seed-order obligation `0! ≤ F(0!)` (`fix_seed_le`) — the base case
...
promoted point is never the cone-zero (its `e_bang`-mass is `one1`) — the
...
`der (0!) = 0`), so `fix_comb (id!) = 0!` — the *diverging value*, which is
provably nonzero.
...
tensor) of the base cones: `EM_prod (!̃X) (!̃Y) ≅ !̃(X & Y)`.
...
of a free type (`tfun t1 t2 ↦ U⟦t1⟧ ⊸ U⟦t2⟧`;
`tprod t1 t2 ↦ sprod (free_base t1) (free_base t2)`) and
`free_decomp t Hfree : coalg_iso (tyD_cbv t) (!̃(free_base t))` witnesses
`tyD_cbv t ≅ !̃(free_base t)`, built by structural induction on the
...
through `fst`/`snd`). Its operational identity is honest divergence: each
...
unit-cone zero (mass `0`), while the pair value itself is `0! ⊗p 0!`
...
context points (`coalg_str Γ γ = γ!` — the §9.7 "γ is a sub-Dirac" reading) in
`theories/programs/infra/cbv_fix_unfold.v`. Writing
`F_γ := curry ⟦body⟧ γ : !L ⊸ !L` for the body's endo-function at `γ`:

- `eD_fix_at_setlike` — the prom-point *computation* law:
  `⟦fix s.body⟧ γ = (fix_value F_γ)!`, i.e. the denotation is the promoted
...
internal hom, a unit-ball chain from `precone_zero` taken as a supremum.
...
the promoted zero `0!` instead, so `Yfix` appears one level down, *inside* the
...
`sup_n fⁿ(0)` is `Yfix_kleeneE`, proved in
...
| Construction | Statement | Rocq |
|---|---|---|
| The β-rule and the if-pins | `(λx.M) V = let x := V in M`, and the boolean dispatch is oriented — `if true` selects the left branch, `if false` the right | `eD_beta`, `eD_if_true`, `eD_if_false` — theories/programs/infra/cbv_anchors.v |
| The let-at-sample integral law | `⟦let x = sample µ in K⟧(γ) = ∫ ⟦K⟧(γ ⊗ δ_r) µ(dr)`, and the same law at an arbitrary bound computation | `eD_let_sample_int`, `eD_let_sample_mu_E`, `eD_let_int` — theories/programs/infra/let_sample_law.v |
| The affine cascade and the sup-mass bridge | The scalar recurrence `x (n+1) = a + q · x n` in closed form, and the mass of a Kleene supremum as the limit of the per-iterate masses | `affine_iter_closed`, `fmeas_kleene_sup_U_E` — theories/programs/infra/affine_cascade.v |
| The setlike-point kit | At setlike unit-ball points the Eq-88 comonoid computes: the diagonal is the pure tensor square, the counit is the unit point | `coalg_d_setlike`, `coalg_str_tensor_setlike`, `em_pair_mor_constE` — theories/programs/infra/cbv_anchors.v |
| The sharing-semantics anchors | `let x = Bernoulli(p) in (x, x)` denotes the diagonal pushforward; two separate draws denote the independent product | `let_bernoulli_pair_diag`, `pair_bernoulli_indep` — theories/programs/infra/cbv_anchors.v |
...
| The conditioning law and the equivalence | `⟦condition m a⟧(U) = ∫_U f dν_M` for an arbitrary model; rejection sampling computes that measure, normalised | `condition_model_E`, `condition_E`, `reject_normalises_condition` — theories/programs/ex_reject_model.v |

### The β-rule and the if-pins (`eD_beta`, `eD_if_true`, `eD_if_false`)

Two morphism-level anchors on the pure fragment. The β-rule
`(λx.M) V = let x := V in M` holds as an equality of `icones_hom`s, not
merely pointwise at chosen environments; the if-pins state that `eD`
sends `if true then M else N` to `⟦M⟧` and `if false then M else N` to
`⟦N⟧`.
...
In the β-rule the `!̃` round trip `der ∘ !(curry M) ∘ str` collapses by
...
> the β-rule and the pins are equalities of `eD_cbv` morphisms rather
...
anywhere, because every step is driven by a genuine `linhom_car` /
...
`eD_let_sample_collapse`, `⟦let x = sample µ in K⟧(γ) = ⟦K⟧(γ ⊗ µ)` — the inner
...
constant); (2) the Dirac approximation `µ = ∫ δ_r µ(dr)` re-spelled with
...
point preserves Pettis integrals, `γ ⊗ (∫ β dµ) = ∫ (γ ⊗ β r) µ(dr)` —
the `linhom_pres_int` field of `τ(γ)` (`ptensor_icone_integral`); and
(4) the `icones_hom_pres_int` field of `⟦K⟧` pushes the denotation under
the integral.
...
sub-distribution `⟦M⟧γ` replaces the constant prior. Unlike the sample
case, the step-1 collapse now genuinely consumes the comonoid copy of
the context (`em_pair_mor id ⟦M⟧` feeds `γ` to *both* legs), so the law
...
The scalar core of every CBV mass headline. Given reals `a, q ≥ 0` and a
real sequence with `x 0 = 0` and `x (n+1) = a + q · x n` — the
per-iterate mass of an affine Kleene chain — the closed form is the
partial geometric series, with the geometric form away from `q = 1` and
...
masses* into the *mass of the Kleene supremum*: for a unit-ball ω-chain
...
`FMeas X`, and both boolean Diracs are setlike.
...
`(sample µ, sample µ)`, denote the independent products `bern ⊗ bern`
and `µ ⊗ µ`. Together the two pairs make the sharing semantics of the
...
> For a general prior `µ`, the shared-`let` pair is the Pettis integral
> `∫ (δ_r ⊗ δ_r) dµ(r)` by the let-at-sample law `eD_let_sample_int`;
...
first projection, and `Lfun_scaleE` (the `linearZ` field of the
...
computation is evaluated at — `one1 ⊗ δ_r`, the context of one
...
`ex_score_posterior_cbv_mass` gives the total evidence `∫ f dµ`. The
...
pairing: at a probability prior (`µ(setT) = 1`), the rejection-sampling
...
> *programs* exactly — not two hand-written measures. The general form,
> with an arbitrary model in place of `sample µ`, is
> `reject_normalises_condition`; this pairing is its historical special
> case at the sampler model.
...
`x` recovers the prior `µ` — *provided `x` is a probability*
...
measurable `U` the iterated-integral measure `∫∫ δ_{m·r0+b}(U) µ(db) µ(dm)` —
...
soft conditioning combinator `condition` (`ex_condition_comb` of
`theories/programs/examples.v`, surface form `Condition { f } M`) takes
a bundled test function `f : testfn` and a model `m : ta → tR`, and
returns the conditioned model
`λa. let x = m a in let _ = Score (test f x) in x`, where `test f` folds
the `[0,1]` test function into `tProb`. The conditioning law states that
at a unit-ball model value `g!` and a setlike unit-ball input `a₀`,
...
The proof engine is the general let-law `eD_let_mu_E` at `ν_M`, with the
score clause computing on Diracs (`eD_score_E` + `score_lift_dirac`) and
...
the readable `⟦·⟧` brackets (Section ReadableHeadlines, over an
...
model evidence `⟦condition_prog⟧(setT)`, itself the content of
`condition_prog_evidence`.

> The historical special case at the sampler model is
> `ex_reject_normalises_score`. On the regression side,
...
> `theories/programs/examples.v`), so the conditioning law and the
> Bayesian-linear evidence are statements about the same operator; the
> full narrative lives on the [Examples tab](../../examples/index.html).
...
norm `‖(p, q)‖ = p + q`. Around that carrier the development runs a
...
| The universal co-pairing | The eliminator `[a, b](x) = bc_t(x)·a + bc_f(x)·b`, with its Dirac equations and its linearity, ω-continuity, norm, path and integral obligations. | `bool_case`, `bool_case_true`, `bool_case_false` — theories/programs/infra/bool_cone.v |
...
true and false, with norm `‖(p, q)‖ = p + q` — concretely the paper
§4.4 / Thm 4.24 coproduct `cone_one_car ⊕ cone_one_car`, with the two
...
> A paper writes the boolean object as the coproduct `1 ⊕ 1` and reads
...
on the two basis points, is linear in `x`, ω-continuous on the unit
...
> obligations — ω-continuity through the sup on the unit ball, path
...
> family is enough whenever the branches are themselves norm-`≤ 1`
...
alpha_linhom a + beta_linhom b` (`bool_case_linhom_gen_alpha_beta`, same
file) — the α/β decomposition that reduces its integral- and
...
integral `Coalg_X(µ) = ∫ prom(δ_x) dµ(x)` degenerates on the two-point
carrier to the finite sum `p·δ_T + q·δ_F ↦ p·prom(δ_T) + q·prom(δ_F)`.
...
to the comonad identities `der ∘ prom = id` and `dig ∘ prom = prom ∘
