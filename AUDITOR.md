...
| Lemma | English statement | Rocq |
|---|---|---|
| Cofree case | On the cofree pair `(!̃A, !̃B)` the η-law `em_pair_mor π₁ π₂ = id` reduces, via `tens_excl_charact`, to a computation on the promoted tensor `x! ⊗ y!`. | `em_pair_mor_proj_id_cofree` — `theories/homs/cbv_adjunction.v` |
...
| Lemma | English statement | Rocq |
|---|---|---|
| The 2-point ICone | A thin record `{nonneg R} × {nonneg R}` with pointwise operations, norm `‖(p, q)‖ = p + q`. Full HB tower `isPrecone` → `isCone` → `isMCone` → `isICone`. Recognised as the paper §4.4 / Thm 4.24 coproduct `cone_one_car ⊕ cone_one_car`. | `bool_cone_car`, `bool_dirac_true`, `bool_dirac_false`, `bool_case` — `theories/cones/bool_cone.v` |
