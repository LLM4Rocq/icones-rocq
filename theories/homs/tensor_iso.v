...
(*                                                                            *)
(*   This file completes the tensor-hom isomorphism by building, on top of    *)
(*   the AXIOM-FREE [tensor_construct] / [tensor_hom_iso] modules, the        *)
(*   inverse [Ψ] of [Φ] as a genuine [icones_hom] and assembling the           *)
(*   isomorphism of integrable cones                                          *)
(*                                                                            *)
(*     [Φ_{B,C,D} : (B⊗C)⊸D  ≅  B⊸(C⊸D)]   (Paper Thm 5.12 / §5.4).            *)
(*                                                                            *)
(*   The mechanism is the paper's: the obstruction that Ψ's value at a        *)
...
(*   - [path_tens_to_X] (Paper [lemma:path-tens-to-one]): a function           *)
(*     [η : X → (B⊗C)⊸D] is a genuine measurable path as soon as it is bounded *)
(*     and measurable ON PURE TENSORS — NO density needed;  (an earlier       *)
(*     scalar-codomain copy, verbatim-superseded by it, was removed);          *)
(*                                                                            *)
...
(** [lfun_path_swap f : ICones(C, Path(X, B⊸D))], assembled from the five
    [lfps] fields by [mk_icones_hom] (Paper [lemma:lfun-path-swap]). *)
...
(** ** The unit iso [1 ⊸ C ≅ C] ([linhom_one_iso])
    ...
    [1 ↔ 1⊸D] bridge in [path_tens_to_X] and for the unitors
    [λ]/[ρ]. *)
...
Definition lo_lift : icones_hom Ar C L :=
  mk_icones_hom (lin_pt (C:=C)) lo_lift_linear lo_lift_continuous
    lo_lift_norm_le1 (fun W g Hg => lo_lift_pres_path (W:=W) (γ:=g) Hg)
    lo_lift_pres_int.
...
(** ** Paper [lemma:path-tens-to-one] at the scalar codomain — ATTICKED

    The original scalar-codomain development ([path_tens_to_one],
    [path_tens_to_one_unit] and their [eta]/[pt_Q] apparatus, Sections
    [PathTensToOne] / [PathTensToOneGen]) used to sit here.  It is
    superseded VERBATIM by the general-codomain sections [PathTensToX] /
    [PathTensToXGen] below (same construction with an arbitrary integrable
    cone [D] in place of the inner scalar [1]) and had no consumers, so it
    was removed.  Paper Lemma 5.11 is delivered by [path_tens_to_X] /
    [path_tens_to_X_unit] (its scalar statement is the [D := 1] instance);
    [docs/PAPER.md] cites those directly. *)
...
(** The composite integrable map [ptX_Q b r : C ⊸ (1⊸D)], the map
...
Definition etaX' : icones_hom Ar B (linhom_car Ar C PX) :=
  mk_icones_hom etaX_inner etaX_outer_linear etaX_outer_continuous
    etaX_outer_norm (fun W δb Hδb => etaX_outer_pres_path (W:=W) (δb:=δb) Hδb)
    etaX_outer_pres_int.
...
(**md**************************************************************************)
(* # STATUS — tensor_hom_iso (this file)                                      *)
(*                                                                            *)
(* DELIVERED (axiom-free; Print Assumptions = the 3 classical [boolp] axioms  *)
(* only):                                                                     *)
(*                                                                            *)
(*  - Pointwise integral-evaluation foundation:                               *)
...
(*                                                                            *)
(*  - Toward Paper [lemma:lfun-path-swap]:                                    *)
...
(*      [ar_prod] reindexing).                                                *)
(*                                                                            *)
(*  - [lfps f : C -> Path(X, B⊸D)] (value [y ↦ MkPath (swap_inner_path f y)]) *)
(*    with [lfpsE] computation, and [lfps_linear] / [lfps_norm_le] /          *)
(*    [lfps_bounded].                                                         *)
(*                                                                            *)
(*  - [lfun_path_swap : icones_hom C (Path(X, B⊸D))]                          *)
...
(*                                                                            *)
(*  - [linhom_one_iso C : icones_iso (1⊸C) C] (the unit iso [1 ⊸ C ≅ C],      *)
(*    Paper §5.5 / Eq 5.3 ingredient), AXIOM-FREE (verified Print Assumptions *)
(*    = 3 boolp):                                                             *)
...
(*    Reused for the [1]↔[1⊸1] bridge AND the unitor [ρ].                     *)
(*                                                                            *)
(*  - [path_tens_to_one] (Paper [lemma:path-tens-to-one] at the SCALAR        *)
(*    codomain [1]) REMOVED: superseded verbatim by [path_tens_to_X] below    *)
(*    and consumed nowhere.  Paper Lemma 5.11 is delivered by                 *)
(*    [path_tens_to_X] / [path_tens_to_X_unit] (scalar case = [D := 1]),      *)
(*    which [docs/PAPER.md] cites directly.                                   *)
(*                                                                            *)
(*  - [path_tens_to_X] (Paper [lemma:path-tens-to-one] at a GENERAL codomain  *)
(*    [D]), AXIOM-FREE (Print Assumptions = 3 boolp).  The scalar-codomain    *)
(*    construction VERBATIM with [D] in place of the inner scalar [1]:        *)
(*    [lin_pt (C:=D)] / [lo_lift (C:=D)] (already general-[C]); the inner     *)
(*    codomain becomes [Path(X, 1⊸D)] ([etaX_*]); [lfun_path_swap] runs with  *)
(*    [C':=1], [D':=D]; [lin_pt_unit] recovers values.  The ONLY change vs    *)
(*    the scalar case is in the three test-driven fields ([ptX_inner_path],   *)
(*    the [etaX]-slot path/integral preservation), where the singleton [1]-   *)
(*    test [id_test] (value [c1_val]) becomes a GENERAL [D]-test [mD] (value  *)
(*    [test_fun mD s]) reindexed via [test_reindex (ar_prod_fst …)] +         *)
(*    [mcone_M_comp]; the pure-tensor hypothesis [ηpt] is stated against an   *)
(*    arbitrary [D]-test.                                                     *)
(*    [Arguments path_tens_to_X {R Ar B C D X} η ηbound ηpt].                 *)
(*                                                                            *)
(*  - [Psi_icones] + [tensor_hom_iso] (MILESTONE), AXIOM-FREE (verified Print *)
(*    Assumptions [tensor_hom_iso] = 3 boolp).                                *)
...
(*      [Arguments tensor_hom_iso {R Ar} B C D].                              *)
(*                                                                            *)
(*  - The four structural isos α/λ/ρ/σ, AXIOM-FREE (Print Assumptions each =  *)
(*    the 3 classical [boolp] axioms).  Built explicitly via [icones_isoP]    *)
(*    (à la [seely.v]) — NOT [yoneda_iso] (the in-file [yoneda_iso] is the    *)
(*    VARY-DOMAIN Yoneda [ICones(-,X)≅ICones(-,Y)], the wrong variance for    *)
(*    the tensor adjunction, which is natural in the CODOMAIN).  The forward  *)
(*    maps are [tensor_uncurry] of an explicit curried datum; the […E] laws   *)
(*    read off by [tensor_curryEp]+[tensor_uncurryK]; round-trips by the      *)
(*    in-module pure-tensor extensionality [tens_ext]/[tens_ext3]/[tens_ext3r]*)
(*    (re-derived from [tensor_curry_inj]+[linhom_tensor_ext]+[tensor_curryEp]*)
(*    — [tensor.v]'s [tensor_ext]/[tensor_ext3] live outside the module):     *)
(*    - [tensor_assoc_iso] + [tensor_assocE] (Eq 5.2): fwd = uncurry∘uncurry  *)
(*      of [Φ∘τ_{A,B⊗C}]; bwd = uncurry of [Ψ∘(double-curry id)].             *)
(*    - [tensor_runit_iso]+[tensor_runitE] (Eq 5.3, ρ): fwd = uncurry         *)
(*      [lo_lift]; bwd = [eval1∘τ_{A,1}]; value [precone_scale (c1_val u) x]. *)
(*    - [tensor_lunit_iso]+[tensor_lunitE] (Eq 5.3, λ): fwd = uncurry of      *)
(*      [u↦(c1_val u)·id_A] ([lin_pt(icones_to_linhom id)]); bwd = [τ_{1,A} 1]*)
(*      as a [linhom_icones].  Unitor round-trips use the in-module           *)
(*      bilinearity [ptensorZl]/[ptensorZr] + the unit-cone fact              *)
(*      [c1one_scaleK : (c1_val u)·1 = u].                                     *)
(*    - the braiding [tensor_braid_iso]+[tensor_braidE] (Eq 5.4), built on    *)
(*      the Fubini swap [swap_lin_lin_hom f : B2→(B1⊸C)] of [f : B1→(B2⊸C)]   *)
...
(*      [linhom_int_eval] + [icone_integral_eqP].  Then [σ_fwd :=             *)
(*      tensor_uncurry (swap_lin_lin_hom (τ_{B,A}))] computes [σ(a⊗b)=b⊗a] by  *)
(*      [tensor_curryEp]+[tensor_uncurryK]+[swap_lin_lin_homE]; backward is   *)
(*      the symmetric swap; round-trips by [tens_ext].                        *)
(*                                                                            *)
(* IMPORTANT — the iso OBJECTS ([tensor_assoc_iso] … and the bifunctor        *)
(* [tensor_mor]) are TRANSPARENT Definitions, so a bare [/=] in downstream     *)
(* COHERENCE proofs ([smcc.v] triangle/pentagon/hexagon/braid-invol; several   *)
(* [seely.v] promotion lemmas) would unfold them into the [tensor_construct]   *)
(* internals and break the [...Ep]/[tensor_morE] rewrites.  Downstream files   *)
(* therefore [Opaque tensor_mor tensor_assoc tensor_lunit tensor_runit         *)
(* tensor_braid] (and [ptensor]/[tau]) after their [...Ep] laws; [conversion]  *)
(* still computes the [...E] values.                                          *)
(******************************************************************************)
