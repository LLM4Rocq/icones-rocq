...
(*                                                                            *)
(*  - [path_tens_to_one] (Paper [lemma:path-tens-to-one]), AXIOM-FREE         *)
(*    (Print Assumptions = 3 boolp).  η : X → (B⊗C)⊸1 BOUNDED + pure-tensor   *)
(*    measurable ([ηpt]: for arity Z, arrow φ:Z→X, B-path β, C-path γ,        *)
(*    [s ↦ c1_val(η(φ s)(β s ⊗ γ s))] measurable on Z) ⇒ a genuine            *)
...
(*      (r ↦ lin_pt(η r (b⊗c))), path [pt_inner_path] via                    *)
(*      [lo_lift_pres_path] + [ηpt] reindexed over [ar_prod]); all 6          *)
(*      [linhom_car] fields proved                                            *)
