...
(** *** Generic glue: a mathcomp-analysis probability on [R] is a
    [pmeas].  The transport itself lives once, at the kernel layer, as
    [distributions.v::pmeas_of_prob] — the [pmeas] packaging of
    [distributions.v::fmeas_of_prob], itself [ppl.v::prob_fmeas] at
    the probability's measure function.  The [sample]-surface name
    here is a plain alias of it, so the two readings are convertible
    and [kernel_anchors.v::pmeas_of_prob_fmeas] is [erefl]. *)
...
Definition pmeas_of_prob : pmeas Ar R_obj :=
  distributions.pmeas_of_prob R_carrier_eq R_to_carrier_meas P.
