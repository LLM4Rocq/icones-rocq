(**md*** Shared infrastructure for the geometric
    ...
    - [geom_coef k = (1/2)^{k+1}] and arithmetic helpers
      [half_nng] / [half_nng_E] / [half_geom_coef_eq].
    - The explicit partial-sum measures [partial_geom n] and
      [partial_geom_shifted n] on [FMeas R_obj].
    - Singleton-measurability and Dirac-PMF helpers
      [R_to_carrier_singleton_meas] / [dirac_fmeas_singleton_eq].
    - The PMF [partial_geom_pmf] and shift-recovery
      [half_shifted_eq].

    Both dist files Require Import this file and consume its lemmas
    directly.
...
(** ** §1 — Coefficient arithmetic on [(1/2)] *)
...
(** [partial_geom_shifted n = Σ_{j < n} (1/2)^{j+1} *: δ_{(j+1)%:R}]. *)
...
(** ** §3 — Singleton measurability + Dirac PMF *)
...
(** ** §4 — PMF of [partial_geom] at integer-valued singletons. *)
...
Arguments geom_coef {R} k.
Arguments half_nng {R}.
Arguments half_nng_E {R}.
Arguments half_geom_coef_eq {R} n.
Arguments partial_geom {R Ar R_obj} R_carrier_eq n.
Arguments partial_geom_shifted {R Ar R_obj} R_carrier_eq n.
Arguments R_to_carrier_singleton_meas
  {R Ar R_obj} R_carrier_eq R_carrier_meas r.
Arguments dirac_fmeas_singleton_eq
  {R Ar R_obj} R_carrier_eq R_carrier_meas a b.
Arguments partial_geom_pmf
  {R Ar R_obj} R_carrier_eq R_carrier_meas n k.
Arguments half_shifted_eq {R Ar R_obj} R_carrier_eq n.
