(**md*** Affine iteration and the geometric series

    A self-contained scalar prelude: the closed form, the geometric
    form and the limit of the affine recurrence

      [x 0 = 0],   [x n.+1 = a + q * x n]     ([a, q >= 0]),

    which is exactly the partial geometric series [series (geometric a q)]
    of mathcomp-analysis.  Nothing here mentions cones; the file wraps
    [geometric_seriesE] and [cvg_geometric_series] into the shape the
    Kleene-iteration arguments of the development consume.

    ** Contents

    - [affine_iter_closed] : the closed form
      [x n = a * (\sum_(i < n) q ^+ i)];
    - [affine_iter_seriesE] : the same, as [series (geometric a q) n];
    ...
    - [affine_iter_deg_eq0] : the degenerate case
      [q = 1 -> a = 0 -> x n = 0];
    - [affine_iter_cvg_half] : the sanity instantiation — at
      [a := 1/2], [q := 1/2] the limit is [1].

    ** Consumers

    The per-iterate masses of an affine Kleene chain satisfy this
    recurrence; the rejection-sampling riders
    ([theories/programs/ex_reject_headline.v],
    [theories/programs/ex_reject_model.v]) instantiate it to close
    their mass limits.
